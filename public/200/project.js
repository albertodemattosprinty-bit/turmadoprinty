...
const project200AppVersion = "0.86";
...
const statsAspectLinksModal = document.getElementById("statsAspectLinksModal");
const statsAspectLinksTitle = document.getElementById("statsAspectLinksTitle");
...
const ilifeFinanceModal = document.getElementById("ilifeFinanceModal");
const ilifeFinanceAddButton = document.getElementById("ilifeFinanceAddButton");
const ilifeFinanceAddMenu = document.getElementById("ilifeFinanceAddMenu");
const ilifeFinanceCurrentBalance = document.getElementById("ilifeFinanceCurrentBalance");
const ilifeFinanceExpectedIncome = document.getElementById("ilifeFinanceExpectedIncome");
...
const ilifeFinanceMonthNext = document.getElementById("ilifeFinanceMonthNext");
const ilifeFinanceMonthLabel = document.getElementById("ilifeFinanceMonthLabel");
const ilifeFinanceEntryWizard = document.getElementById("ilifeFinanceEntryWizard");
const ilifeFinanceWizardClose = document.getElementById("ilifeFinanceWizardClose");
const ilifeFinanceWizardKicker = document.getElementById("ilifeFinanceWizardKicker");
const ilifeFinanceWizardTitle = document.getElementById("ilifeFinanceWizardTitle");
const ilifeFinanceDetailsStep = document.getElementById("ilifeFinanceDetailsStep");
const ilifeFinanceScheduleStep = document.getElementById("ilifeFinanceScheduleStep");
const ilifeFinanceTitleInput = document.getElementById("ilifeFinanceTitleInput");
const ilifeFinanceAmountInput = document.getElementById("ilifeFinanceAmountInput");
const ilifeFinanceFutureHint = document.getElementById("ilifeFinanceFutureHint");
const ilifeFinanceDetailsStatus = document.getElementById("ilifeFinanceDetailsStatus");
const ilifeFinanceDetailsNext = document.getElementById("ilifeFinanceDetailsNext");
const ilifeFinanceScheduleQuestion = document.getElementById("ilifeFinanceScheduleQuestion");
const ilifeFinanceModeLabel = document.getElementById("ilifeFinanceModeLabel");
const ilifeFinanceModeHint = document.getElementById("ilifeFinanceModeHint");
const ilifeFinanceFrequencyRow = document.getElementById("ilifeFinanceFrequencyRow");
const ilifeFinanceFrequencyLabel = document.getElementById("ilifeFinanceFrequencyLabel");
const ilifeFinanceValueRow = document.getElementById("ilifeFinanceValueRow");
const ilifeFinanceValueLabel = document.getElementById("ilifeFinanceValueLabel");
const ilifeFinanceValueHint = document.getElementById("ilifeFinanceValueHint");
const ilifeFinanceOnceDateWrap = document.getElementById("ilifeFinanceOnceDateWrap");
const ilifeFinanceOnceDate = document.getElementById("ilifeFinanceOnceDate");
const ilifeFinanceEndDateWrap = document.getElementById("ilifeFinanceEndDateWrap");
const ilifeFinanceEndDate = document.getElementById("ilifeFinanceEndDate");
const ilifeFinanceCustomOpen = document.getElementById("ilifeFinanceCustomOpen");
const ilifeFinanceWizardStatus = document.getElementById("ilifeFinanceWizardStatus");
const ilifeFinanceScheduleBack = document.getElementById("ilifeFinanceScheduleBack");
const ilifeFinanceSave = document.getElementById("ilifeFinanceSave");
const ilifeFinanceCustomModal = document.getElementById("ilifeFinanceCustomModal");
const ilifeFinanceCustomTitle = document.getElementById("ilifeFinanceCustomTitle");
const ilifeFinanceCustomMonthly = document.getElementById("ilifeFinanceCustomMonthly");
const ilifeFinanceCustomWeekly = document.getElementById("ilifeFinanceCustomWeekly");
const ilifeFinanceCustomDaily = document.getElementById("ilifeFinanceCustomDaily");
const ilifeFinanceMonthDays = document.getElementById("ilifeFinanceMonthDays");
const ilifeFinanceWeekDays = document.getElementById("ilifeFinanceWeekDays");
const ilifeFinanceCustomDateInput = document.getElementById("ilifeFinanceCustomDateInput");
const ilifeFinanceCustomDateAdd = document.getElementById("ilifeFinanceCustomDateAdd");
const ilifeFinanceCustomDates = document.getElementById("ilifeFinanceCustomDates");
...
const missionsEmptyText = document.getElementById("missionsEmptyText");
...
const missionCreateConfirmLabel = document.getElementById("missionCreateConfirmLabel");
const missionCreateBackButton = document.getElementById("missionCreateBack");
const missionCreateSlider = document.getElementById("missionCreateSlider");
...
function normalizeActiveTimeMinutes(value, fallback, allowEndOfDay = false) {
  ...
  return Number.isFinite(numeric) ? Math.max(0, Math.min(maximum, numeric)) : fallback;
}

function clockInputToMinutes(value, { end = false } = {}) {
  const match = String(value || "").match(/^(\d{2}):(\d{2})$/);
  if (!match) return end ? 1440 : 480;
  const minutes = Math.max(0, Math.min(1439, (Number(match[1]) * 60) + Number(match[2])));
  return end && minutes === 0 ? 1440 : minutes;
}

function activeMinutesToInput(minutes) {
  const safe = Math.max(0, Math.min(1440, Number(minutes || 0)));
  if (safe === 1440) return "00:00";
  return `${String(Math.floor(safe / 60)).padStart(2, "0")}:${String(safe % 60).padStart(2, "0")}`;
}

function activeMinutesToLabel(minutes) {
  const value = Number(minutes) === 1440 ? 0 : Math.max(0, Number(minutes || 0));
  return `${String(Math.floor(value / 60)).padStart(2, "0")}h${value % 60 ? String(value % 60).padStart(2, "0") : ""}`;
}

function normalizeMissionKind(value) {
  return String(value || "goal").trim().toLowerCase() === "limit" ? "limit" : "goal";
}

function isLimitGoal(goal) {
  return normalizeMissionKind(goal?.goalKind) === "limit";
}

function getActiveTimeDurationMinutes(settings = state.activeTime) {
  const start = normalizeActiveTimeMinutes(settings?.startMinutes, 480);
  const end = normalizeActiveTimeMinutes(settings?.endMinutes, 1440, true);
  return end > start ? end - start : (1440 - start) + end;
}

function getActiveTimeWindow(nowMs = getServerNowMs()) {
  const startMinutes = normalizeActiveTimeMinutes(state.activeTime?.startMinutes, 480);
  const endMinutes = normalizeActiveTimeMinutes(state.activeTime?.endMinutes, 1440, true);
  const durationMinutes = getActiveTimeDurationMinutes({ startMinutes, endMinutes });
  const dateKey = getProjectDateKey(new Date(nowMs));
  let dayStartMs = projectDateKeyToDate(dateKey).getTime();
  let startMs = dayStartMs + (startMinutes * 60000);
  let endMs = startMs + (durationMinutes * 60000);
  if (endMinutes <= startMinutes && nowMs < dayStartMs + (endMinutes * 60000)) {
    startMs -= 86400000;
    endMs -= 86400000;
  }
  return { startMs, endMs, startMinutes, endMinutes, durationMinutes, dateKey: getProjectDateKey(new Date(startMs)) };
}

function getMissionExpectation(goal, nowMs = getServerNowMs()) {
...
function getMissionInstallmentVisual(delayMinutes) {
  const delay = Number(delayMinutes || 0);
  if (delay <= 15) return { className: "mission-dynamic-blue", rgb: "36, 97, 206" };
  if (delay <= 30) return { className: "mission-dynamic-gradient", rgb: mixMissionRgb([36, 97, 206], [255, 214, 61], (delay - 15) / 15) };
  if (delay <= 60) return { className: "mission-dynamic-gradient", rgb: mixMissionRgb([255, 214, 61], [255, 151, 45], (delay - 30) / 30) };
  if (delay <= 180) return { className: "mission-dynamic-gradient", rgb: mixMissionRgb([255, 151, 45], [226, 59, 59], (delay - 60) / 120) };
  if (delay <= 360) return { className: "mission-dynamic-extreme", rgb: "226, 59, 59", darkness: Math.max(0, Math.min(1, (delay - 180) / 180)) };
...
async function loadProject200ActiveTime() {
  if (!getToken()) return;
  try {
    const payload = await apiRequest("/api/200/active-time", { skipGlobalLoading: true });
    state.activeTime = {
      startMinutes: normalizeActiveTimeMinutes(payload?.activeTime?.startMinutes, 480),
      endMinutes: normalizeActiveTimeMinutes(payload?.activeTime?.endMinutes, 1440, true),
      loaded: true,
      saving: false
    };
  } catch {
    state.activeTime = { startMinutes: 480, endMinutes: 1440, loaded: true, saving: false };
  }
  renderOptionsModal();
}

async function requestMissionNotificationPermission() {
  const notifications = window.Capacitor?.Plugins?.LocalNotifications || null;
  if (!notifications?.requestPermissions) return false;
  try {
...
function missionNotificationId(value) {
  let hash = 0;
...
function getMetricScopeFromDays(days) {
  const safeDays = Math.max(1, Math.trunc(Number(days || 1)));
  if (safeDays === 1) return { key: "today", label: "Hoje", days: 1 };
...
function buildIlifeFinanceWizard(kind) {
...
function getIlifeFinanceMonthKey() {
  const date = state.ilifeFinance.monthCursor;
  return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}`;
}

function getIlifeFinanceMonthName() {
  return new Intl.DateTimeFormat("pt-BR", { month: "long", year: "numeric" }).format(state.ilifeFinance.monthCursor);
}
...
function setIlifeFinanceToast(message) {
  const toast = getIlifeFinanceEl("ilifeFinanceToast");
  if (!toast) return;
  toast.textContent = message;
  toast.classList.add("is-visible");
  window.clearTimeout(state.ilifeFinance.toastTimer);
  state.ilifeFinance.toastTimer = window.setTimeout(() => toast.classList.remove("is-visible"), 2400);
}
...
function renderIlifeFinanceSummary() {
  ...
  const currentBalance = getIlifeFinanceEl("ilifeFinanceCurrentBalance");
  const projectedBalance = getIlifeFinanceEl("ilifeFinanceProjectedBalance");
  const expectedIncome = getIlifeFinanceEl("ilifeFinanceExpectedIncome");
  const expectedExpense = getIlifeFinanceEl("ilifeFinanceExpectedExpense");
  const freeMoney = getIlifeFinanceEl("ilifeFinanceFreeMoney");
  const incomeMonth = getIlifeFinanceEl("ilifeFinanceIncomeMonth");
  const expenseMonth = getIlifeFinanceEl("ilifeFinanceExpenseMonth");
  const healthPill = getIlifeFinanceEl("ilifeFinanceHealthPill");
  ...
  const today = getProjectTodayDateKey();
  const attentionEntries = (Array.isArray(summary.attentionEntries) ? summary.attentionEntries : [])
...
async function loadIlifeFinanceLedger() {
  if (getIlifeFinanceEl("ilifeFinanceMonthLabel")) getIlifeFinanceEl("ilifeFinanceMonthLabel").textContent = getIlifeFinanceMonthName();
  if (!getToken()) {
...
function setIlifeFinanceAddMenu() {
  state.ilifeFinance.addMenuOpen = false;
}

function moveIlifeFinanceMonth(direction) {
  const current = state.ilifeFinance.monthCursor;
  state.ilifeFinance.monthCursor = new Date(current.getFullYear(), current.getMonth() + direction, 1);
  renderIlifeFinanceSummary();
  void loadIlifeFinanceLedger();
}

function openIlifeFinanceWizard(kind) {
  state.ilifeFinance.wizard = buildIlifeFinanceWizard(kind);
...
function closeIlifeFinanceWizard() {
  if (ilifeFinanceEntryWizard) {
    ilifeFinanceEntryWizard.classList.remove("is-open");
    ilifeFinanceEntryWizard.hidden = true;
    ilifeFinanceEntryWizard.setAttribute("aria-hidden", "true");
  }
...
async function saveIlifeFinanceItem() {
  ...
  if (payload.endsOn && payload.endsOn < payload.startsOn) {
    if (ilifeFinanceWizardStatus) ilifeFinanceWizardStatus.textContent = "Escolha uma data final depois da data inicial.";
    return;
  }
...
function closeModal(modal) {
  ...
  if (modal.id === "historyModal") {
    if (missionHealthTicker) {
      window.clearInterval(missionHealthTicker);
...
function renderActions() {
  ...
  timelineEntries.forEach((action, actionIndex) => {
    ...
    if (action.kind === "mission-installment") {
      const visual = getMissionInstallmentVisual(action.delayMinutes);
      const missionIcon = getMissionDisplayIcon(action.goal);
      const row = document.createElement("article");
      row.className = `task-row task-mission-dynamic ${visual.className}`;
      row.dataset.missionGoalId = action.goalId;
      row.dataset.missionInstallment = String(action.installmentNumber);
      row.style.setProperty("--mission-delay-rgb", visual.rgb);
...
function buildStatsPointEntry(category, byCategory = {}, configOverride = null) {
  ...
  const missionGoals = isSleepStatsCategory(category.id)
    ? []
    : scopedMissions.filter((goal) => !isLimitGoal(goal) && selectedGoalIds.has(String(goal.id || "").trim()));
...
function getStatsAspectActionRange() {
  const scopeDays = getStatsScopeDays();
  const today = new Date(getServerNowMs());
  const firstDay = addDays(today, -(scopeDays - 1));
  return {
    from: startOfDayIso(firstDay),
    to: nextDayIso(today)
  };
}
...
function getStatsAspectLinkedMissions(categoryId) {
  ...
  return (Array.isArray(state.missions) ? state.missions : [])
    .filter((goal) => !isLimitGoal(goal) && selectedIds.has(String(goal?.id || "").trim()));
}
...
async function openStatsAspectLinksModal(categoryId) {
  const normalizedCategoryId = normalizeTaskCategoryId(categoryId);
  ...
  try {
    const range = getStatsAspectActionRange();
    const payload = await apiRequest(`/api/actions?from=${encodeURIComponent(range.from)}&to=${encodeURIComponent(range.to)}`, {
      skipGlobalLoading: true
    });
    const actions = (Array.isArray(payload?.actions) ? payload.actions : [])
      .filter((action) => normalizeAssigneeName(action?.assignee) === state.selectedProfile)
      .filter((action) => normalizeTaskCategoryId(action?.categoryId) === normalizedCategoryId)
      .sort((left, right) => new Date(left?.startAt || 0).getTime() - new Date(right?.startAt || 0).getTime());
    if (String(state.statsAspectLinks?.categoryId || "") !== normalizedCategoryId) return;
    state.statsAspectLinks.actions = actions;
  } catch {
    const fallbackActions = getVisibleActions()
      .filter((action) => normalizeTaskCategoryId(action?.categoryId) === normalizedCategoryId)
      .sort((left, right) => new Date(left?.startAt || 0).getTime() - new Date(right?.startAt || 0).getTime());
    if (String(state.statsAspectLinks?.categoryId || "") !== normalizedCategoryId) return;
    state.statsAspectLinks.actions = fallbackActions;
  } finally {
    if (String(state.statsAspectLinks?.categoryId || "") === normalizedCategoryId) {
...
async function showPointsUpdateFeedback(pointsUpdate) {
  ...
  pointsUpdateModal.classList.remove("is-leaving", "is-overtaking", "is-ready-to-continue", "is-loss", "is-solo-points");
  if (pointsUpdateContinue) pointsUpdateContinue.hidden = true;
}
...
function renderMissionKindFilter() {
  const showingLimits = state.missionKindFilter === "limit";
  if (missionKindToggle) {
    missionKindToggle.classList.toggle("is-limit", showingLimits);
    missionKindToggle.setAttribute("aria-pressed", showingLimits ? "true" : "false");
    missionKindToggle.setAttribute("aria-label", showingLimits ? "Mostrar metas" : "Mostrar limites");
  }
  if (missionKindToggleLabel) missionKindToggleLabel.textContent = showingLimits ? "Limites" : "Metas";
  if (missionKindIcon) missionKindIcon.innerHTML = showingLimits
    ? "<svg viewBox=\"0 0 24 24\" aria-hidden=\"true\"><path d=\"M8.2 11V5.8a1.35 1.35 0 0 1 2.7 0V10 4.7a1.35 1.35 0 0 1 2.7 0V10 5.3a1.35 1.35 0 0 1 2.7 0v5.2-3.6a1.35 1.35 0 0 1 2.7 0v6.7c0 4.1-2.6 7-6.7 7H11c-2.2 0-3.7-.9-5.1-2.5L3.4 15a1.45 1.45 0 0 1 2.1-2l2.7 2.1V11Z\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.7\" stroke-linecap=\"round\" stroke-linejoin=\"round\"/></svg>"
...
function renderMissionCreateStep(direction = 0) {
  ...
  if (direction !== 0) {
    ...
    window.setTimeout(() => active?.classList.remove("slide-from-right", "slide-from-left"), 360);
  }
}
...
async function preloadMissionRunDailyRanking() {
  const goal = getMissionRunGoalById(state.missionRun?.goalId);
  if (state.missionRun.rankingPreloadStarted || !getToken() || isLimitGoal(goal)) return;
...
function openMissionProgressModal(goalId) {
  ...
  if (missionProgressIcon) {
    ...
    missionProgressIcon.alt = goalIcon.alt;
  }
...
function interpolateLimitColor(ratio) {
  const stops = [
    { at: 0, rgb: [37, 212, 119] },
    { at: 100, rgb: [75, 140, 255] },
    { at: 120, rgb: [255, 214, 61] },
    { at: 150, rgb: [255, 151, 45] },
    { at: 200, rgb: [226, 59, 59] }
  ];
  const safe = Math.max(0, Math.min(200, Number(ratio || 0)));
  const upperIndex = Math.max(1, stops.findIndex((stop) => safe <= stop.at));
  const lower = stops[upperIndex - 1];
  const upper = stops[upperIndex];
  const amount = upper.at === lower.at ? 0 : (safe - lower.at) / (upper.at - lower.at);
  return `rgb(${mixMissionRgb(lower.rgb, upper.rgb, amount)})`;
}
...
function formatMissionRangeProgress(progress, days) {
  ...
  return `${total} ${total === 1 ? "Vez" : "Vezes"} | ${average.toLocaleString("pt-BR", { minimumFractionDigits: 1, maximumFractionDigits: 1 })} por dia`;
}
...
function createMissionCard(goal, initialPercent = null) {
  ...
  const regularPercent = dailyVariantProgress ? dailyVariantProgress.percent : Math.max(0, Math.min(100, Math.round((progress / target) * 100)));
  ...
  const delayVisual = expectation && expectation.delayMinutes > 60 ? getMissionInstallmentVisual(expectation.delayMinutes) : null;
  card.className = `history-mission-card${limit ? " is-limit" : ""}${hasMicrotasks ? " has-microtasks" : ""}${delayVisual ? ` mission-overdue-container ${delayVisual.className}` : ""}`;
  if (delayVisual) {
  ...
  card.innerHTML = `
    ...
    <div class="history-mission-progress-track" aria-hidden="true"><div class="history-mission-progress-fill" style="width:${safeInitialPercent}%;${limitVisual ? `background:${limitVisual.background};` : ""}"></div></div>
  `;
  return card;
}
...
function renderMissions() {
  ...
  const showingLimits = state.missionKindFilter === "limit";
  const goals = allGoals.filter((goal) => isLimitGoal(goal) === showingLimits);
  ...
  if (!missionList || !goals.length) return;
  goals.forEach((goal) => {
    const goalId = String(goal?.id || "");
    const priorPercent = previousProgress.get(goalId);
    ...
    if (Number.isFinite(priorPercent) && Number.isFinite(nextPercent) && Math.abs(priorPercent - nextPercent) >= 0.1) animateProgressFillWidth(card.querySelector(".history-mission-progress-fill"), nextPercent, priorPercent);
  });
}
...
async function bootstrapProject200App() {
  try {
    ...
    await loadProject200ActiveTime();
...
document.querySelectorAll("[data-open-modal]").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    if (loadingIcon) globalLoadingPreferredIcon = loadingIcon;
...
getIlifeFinanceEl("ilifeFinanceMonthToday")?.addEventListener("click", () => {
  const now = new Date();
  state.ilifeFinance.monthCursor = new Date(now.getFullYear(), now.getMonth(), 1);
  void loadIlifeFinanceLedger();
});
...
ilifeFinanceEntryWizard?.addEventListener("click", (event) => {
  if (event.target === ilifeFinanceEntryWizard) closeIlifeFinanceWizard();
});
...
toggleMissionActionsOptionButton?.classList.toggle("is-off", missionActionsMode === "hide");
  if (activeTimeStartInput && document.activeElement !== activeTimeStartInput) activeTimeStartInput.value = activeMinutesToInput(state.activeTime.startMinutes);
  if (activeTimeEndInput && document.activeElement !== activeTimeEndInput) activeTimeEndInput.value = activeMinutesToInput(state.activeTime.endMinutes);
...
let statsAspectHoldTriggered = false;
let statsAspectHeldCategoryId = "";
statsMissionsList?.addEventListener("pointerdown", (event) => {
  const row = event.target.closest("[data-stats-aspect-id]");
  if (!row) return;
  if (event.pointerType === "mouse" && event.button !== 0) return;
  const categoryId = String(row.dataset.statsAspectId || "").trim().toLowerCase();
  statsAspectHoldTriggered = false;
  statsAspectHeldCategoryId = categoryId;
  window.clearTimeout(statsAspectHoldTimer);
  statsAspectHoldTimer = window.setTimeout(() => {
    statsAspectHoldTriggered = true;
    if (categoryId === "sono") {
      void openSleepHistoryEditor();
      return;
    }
    void openStatsAspectLinksModal(categoryId);
  }, 500);
});
["pointerup", "pointerleave", "pointercancel"].forEach((eventName) => {
  statsMissionsList?.addEventListener(eventName, () => {
    window.clearTimeout(statsAspectHoldTimer);
    statsAspectHoldTimer = 0;
  });
});
statsMissionsList?.addEventListener("contextmenu", (event) => {
  const row = event.target.closest("[data-stats-aspect-id]");
  if (!row) return;
  event.preventDefault();
  window.clearTimeout(statsAspectHoldTimer);
  statsAspectHoldTimer = 0;
  const categoryId = String(row.dataset.statsAspectId || "").trim().toLowerCase();
  if (statsAspectHoldTriggered && statsAspectHeldCategoryId === categoryId) {
    return;
  }
  statsAspectHoldTriggered = false;
  statsAspectHeldCategoryId = "";
  if (categoryId === "sono") {
    void openSleepHistoryEditor();
    return;
  }
...
statsMissionsList?.addEventListener("click", (event) => {
  ...
  if (statsAspectHoldTriggered && String(row.dataset.statsAspectId || "") === statsAspectHeldCategoryId) {
    statsAspectHoldTriggered = false;
    statsAspectHeldCategoryId = "";
    return;
  }
...
missionKindToggle?.addEventListener("click", () => {
  state.missionKindFilter = state.missionKindFilter === "limit" ? "goal" : "limit";
  renderMissions();
});
...
toggleMissionActionsOptionButton?.addEventListener("click", () => {
  void (async () => {
    ...
    state.actionsMissionOnly = false;
    state.actionsDynamicMissionsVisible = true;
    if (state.options.missionActionsMode === "dynamic" && !state.options.missionNotificationsEnabled) {
      state.options.missionNotificationsEnabled = await requestMissionNotificationPermission();
    }
    saveOptionsConfig();
    renderOptionsModal();
    renderActions();
...
saveActiveTimeButton?.addEventListener("click", () => {
  void (async () => {
    ...
    try {
      ...
      state.activeTime.saving = false;
      renderOptionsModal();
      if (activeTimeStatus) activeTimeStatus.textContent = error instanceof Error ? error.message : "Falha ao salvar.";
    }
  })();
});
...
(function installProject200UniversalSchedule() {
  ...
  function formatAvoidFeedback(days) {
    const clean = normalizeAvoidDays(days, []);
    if (!clean.length) return "Nenhum dia evitado";
    if (clean.length === 1) return `Evitar ${avoidDayNames[clean[0]]}`;
    if (clean.length === 2) return `Evitar ${avoidDayNames[clean[0]]} e ${avoidDayNames[clean[1]]}`;
    return `Evitando ${clean.length} dias`;
  }
  function ensureFields() {
    state.wizard.repeatInterval = Math.max(1, Number(state.wizard.repeatInterval || 1));
    state.wizard.repeatIntervalUnit = ["day", "week", "month", "year"].includes(state.wizard.repeatIntervalUnit) ? state.wizard.repeatIntervalUnit : "day";
  ...
  function applyDataSelectModal() {
    const target = dataSelectTarget || "mission-create";
    const cfg = window.project200MissionScheduleBridge?.read(target) || {};
    cfg.startsOn = formatDateSelectKey(dataSelectDraft);
    syncSimpleScheduleFromStartDate(cfg);
    window.project200MissionScheduleBridge?.save(cfg, target);
  ...
  function ensureModal() {
    ...
    modal.addEventListener("click", function(event) {
      ...
      const avoidDay = event.target.closest("[data-us-avoid-day]");
      if (avoidDay) { const set = new Set(normalizeAvoidDays(state.wizard.repeatAvoidDays, [0, 6])); const value = Number(avoidDay.dataset.usAvoidDay); set.has(value) ? set.delete(value) : set.size < 6 && set.add(value); state.wizard.repeatAvoidDays = Array.from(set).sort((a,b)=>a-b); renderModal(); }
    });
    modal.addEventListener("change", function(event) {
      if (event.target.matches("[name='usEnd']")) { state.wizard.repeatEndMode = event.target.value; renderModal(); }
    });
  ...
  function setRepeatChoice(mode) {
    ...
    if (mode === "periodic") { state.wizard.repeatIntervalUnit = "day"; state.wizard.repeatInterval = Math.max(2, Number(state.wizard.repeatInterval || state.wizard.periodicEveryDays || 2)); state.wizard.repeatAvoidDays = normalizeAvoidDays(state.wizard.repeatAvoidDays, [0, 6]); }
  }
  function renderModal() {
    ensureFields();
    const modal = ensureModal();
    const mode = state.wizard.repeatOpen ? normalizeRepeatMode(state.wizard.repeatMode) : "daily";
    const isDailyOnly = mode === "daily" && state.wizard.repeatIntervalUnit === "day" && Number(state.wizard.repeatInterval || 1) === 1;
    ...
    modal.querySelector("[data-us-details]").hidden = isDailyOnly;
    modal.querySelector("[data-us-custom]").hidden = !["periodic", "weekly", "monthly_custom"].includes(mode);
    const customLabel = modal.querySelector("[data-us-custom-label]");
    if (customLabel) customLabel.textContent = mode === "periodic" ? "A cada" : mode === "weekly" ? "Dias da semana" : "Mensalmente";
    const periodicOnly = modal.querySelector("[data-us-periodic-only]");
    modal.querySelectorAll("[data-us-periodic-only]").forEach((element) => { element.hidden = mode !== "periodic"; });
    const avoidWrap = modal.querySelector("#usAvoidDays");
    if (avoidWrap) avoidWrap.innerHTML = weekdays.map((label, day) => '<button type="button" data-us-avoid-day="' + day + '" class="' + (normalizeAvoidDays(state.wizard.repeatAvoidDays, [0, 6]).includes(day) ? 'active' : '') + '">' + label + '</button>').join('');
    const avoidFeedback = modal.querySelector("#usAvoidFeedback");
    if (avoidFeedback) avoidFeedback.textContent = formatAvoidFeedback(state.wizard.repeatAvoidDays);
    modal.querySelector("[data-us-end]").hidden = mode === "none" || isDailyOnly;
    modal.querySelector("#usInterval").value = state.wizard.repeatInterval;
    const usUnit = modal.querySelector("#usUnit");
    if (usUnit) usUnit.value = state.wizard.repeatIntervalUnit;
    const startKey = getProjectDateKey(dateFromOffset(state.wizard.dateOffset));
    modal.querySelector("#usStart").value = startKey;
    const todayKey = getProjectDateKey(new Date(getServerNowMs ? getServerNowMs() : Date.now()));
    const startLabel = modal.querySelector("[data-us-start-label]");
    if (startLabel) startLabel.textContent = startKey === todayKey ? "Comecar em: Hoje" : "Comecar em";
    ...
    modal.querySelectorAll("[data-us-end-extra]").forEach((el) => { el.hidden = state.wizard.repeatEndMode === "never"; });
    modal.querySelector("#usEndDate").hidden = state.wizard.repeatEndMode !== "date";
    modal.querySelector("#usEndCount").closest(".universal-schedule-inline").hidden = state.wizard.repeatEndMode !== "count";
  ...
  function applyModal() {
    const modal = ensureModal();
    const selectedMode = modal.querySelector("[data-us-repeat].active")?.dataset.usRepeat || "daily";
    ...
    if (selectedMode === "daily") {
      state.wizard.repeatOpen = true;
      state.wizard.repeatMode = "daily";
      state.wizard.repeatInterval = 1;
      state.wizard.repeatIntervalUnit = "day";
      state.startDecisionContext.dirty = true;
      closeModalLocal();
      renderTaskComposerModal();
      return;
    ...
    state.wizard.repeatIntervalUnit = selectedMode === "periodic" ? "day" : (modal.querySelector("#usUnit")?.value || state.wizard.repeatIntervalUnit || "day");
  ...
  buildOccurrences = function() {
    ...
    const avoidDays = state.wizard.repeatMode === "periodic" ? normalizeAvoidDays(state.wizard.repeatAvoidDays, []) : [];
    if (state.wizard.repeatIntervalUnit === "day") for (let i = 0; i < 730 && out.length < max; i += interval) { const date = addDays(selected, i); if (!avoidDays.includes(date.getDay())) push(date); }
...
(function installProject200UniversalScheduleMissionsBridge() {
  ...
  function baseSchedule(days = [getMissionLocalWeekday()]) {
    ...
    return { frequency: "daily", interval: 1, intervalUnit: "day", weekDays: cleanDays(days, ALL_MISSION_REPEAT_DAYS), monthlyMode: "weekday", monthDay: new Date().getDate(), monthlyOrdinalIndex: 0, monthlyWeekdayIndex: getMissionLocalWeekday(), startsOn: today, endMode: "never", endsOn: "", count: 10, avoidDays: [0, 6], notification: { mode: "at_time", customAmount: 10, customUnit: "minutes" } };
  }
  function normalizeSchedule(config, fallbackDays = []) {
    const raw = config && typeof config === "object" ? config : {};
    const merged = { ...baseSchedule(fallbackDays), ...raw };
    merged.frequency = ["none", "daily", "weekly", "monthly_custom", "periodic", "yearly"].includes(String(merged.frequency || "")) ? String(merged.frequency) : "daily";
    ...
    merged.avoidDays = normalizeAvoidDays(merged.avoidDays, [0, 6]);
  ...
  window.project200MissionScheduleBridge = {
    read: readTargetSchedule,
    save: saveTargetSchedule
  };
  ...
  const repeatUntilMonthNames = ["janeiro", "fevereiro", "marco", "abril", "maio", "junho", "julho", "agosto", "setembro", "outubro", "novembro", "dezembro"];
  let repeatUntilDraft = null;
  function repeatUnitLabel(unit, interval) { const pairs = { day:["Dia","Dias"], week:["Semana","Semanas"], month:["Mes","Meses"], year:["Ano","Anos"] }; const pair = pairs[unit] || pairs.day; return Number(interval || 1) === 1 ? pair[0] : pair[1]; }
  function getScheduleDateParts(schedule) { const key = String(schedule?.startsOn || getProjectDateKey(new Date(getServerNowMs()))).slice(0,10); const match = key.match(/^(\d{4})-(\d{2})-(\d{2})$/); if (!match) { const now = new Date(getServerNowMs()); return { month:now.getMonth()+1, day:now.getDate(), weekday:now.getDay(), ordinal:0 }; } const year = Number(match[1]), month = Number(match[2]), day = Number(match[3]); return { month, day, weekday:new Date(year,month-1,day,12).getDay(), ordinal:day + 7 > new Date(year,month,0).getDate() ? 4 : Math.floor((day-1)/7) }; }
  function getMonthlyPositionLabel(schedule) { const parts = getScheduleDateParts(schedule); return "Na " + (ordinals[parts.ordinal] || ordinals[0]) + " " + (weekdayLong[parts.weekday] || weekdayLong[1]) + " de " + (repeatUntilMonthNames[parts.month-1] || "mes"); }
  function formatEndSummary(schedule) { const cfg = normalizeSchedule(schedule); if (cfg.endMode === "count") return "Depois de " + cfg.count + " repeticoes"; if (cfg.endMode === "date" && cfg.endsOn) return "Ate " + formatDateSelectLong(cfg.endsOn); return "Para sempre"; }
  function updateIntervalLabels(modal, unit, interval) { const safeUnit = ["day","week","month","year"].includes(unit) ? unit : "day"; const safeInterval = Math.max(1, Math.trunc(Number(interval || 1) || 1)); const select = modal.querySelector("#usbUnit"); if (select) { select.querySelectorAll("option").forEach((option) => option.textContent = repeatUnitLabel(option.value, safeInterval)); select.value = safeUnit; } const monthEvery = modal.querySelector("#usbMonthlyEveryLabel"); if (monthEvery) monthEvery.textContent = "Repetir a cada " + safeInterval + " " + repeatUnitLabel("month",safeInterval).toLowerCase(); }
  ...
  function ensureModal() {
    ...
    if (modal) return modal;
    modal = document.createElement("section"); modal.id = "dailyrepetitionModal"; modal.dataset.modalName = "dailyrepetition"; modal.setAttribute("aria-label","dailyrepetition"); modal.className = "workspace-modal simple-modal universal-schedule-modal"; modal.setAttribute("aria-hidden","true");
    modal.innerHTML = '<div class="universal-schedule-panel"><header class="universal-schedule-head"><h2 class="universal-schedule-title">Quando se repete?</h2><button class="history-mission-modal-close" type="button" data-usb-close aria-label="Fechar">x</button></header><div class="universal-schedule-body"><section class="universal-schedule-card"><label>Repeticao</label><div class="universal-schedule-options"><button type="button" data-usb-repeat="none">Nunca se repete</button><button type="button" data-usb-repeat="daily">Todos os dias</button><button type="button" data-usb-repeat="weekly">Todas as semanas</button><button type="button" data-usb-repeat="monthly_custom">Todos os meses</button><button type="button" data-usb-repeat="yearly">Todos os anos</button><button type="button" data-usb-repeat="periodic">Personalizado</button></div><p class="dailyrepetition-feedback" id="usbRepeatFeedback"></p></section><section class="universal-schedule-card" id="usbEndCard"><label>Repete ate</label><button class="dailyrepetition-date-button" type="button" id="usbEndButton"><span id="usbEndButtonText">Para sempre</span><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m9 5 7 7-7 7" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg></button></section><div data-usb-details><section class="universal-schedule-card" data-usb-custom><label>Repetir a cada</label><div class="universal-schedule-inline universal-schedule-periodic-inline"><input type="number" min="1" max="999" id="usbInterval"><select id="usbUnit"><option value="day">Dia</option><option value="week">Semana</option><option value="month">Mes</option><option value="year">Ano</option></select></div></section><section class="universal-schedule-card universal-schedule-avoid" data-usb-day-only><label>Evitar:</label><div class="universal-weekdays universal-weekdays-avoid" id="usbAvoidDays"></div><small id="usbAvoidFeedback">Evitar sabados e domingos</small></section><section class="universal-schedule-card" data-usb-week-only><label>Dias da semana</label><div class="universal-weekdays" id="usbWeekdays"></div></section><section class="universal-schedule-card" id="usbMonthly"><label>Repeticao mensal</label><label><input type="radio" name="usbMonthlyMode" value="day"> <span id="usbMonthlyEveryLabel">Repetir a cada 1 mes</span></label><label><input type="radio" name="usbMonthlyMode" value="weekday"> <span id="usbMonthlyPositionLabel">Na primeira segunda-feira de janeiro</span></label></section><section class="universal-schedule-card"><label>Notificacao</label><select id="usbNotification"><option value="at_time">No horario da tarefa</option><option value="5m">5 minutos antes</option><option value="10m">10 minutos antes</option><option value="30m">30 minutos antes</option><option value="1h">1 hora antes</option><option value="1d">1 dia antes</option><option value="custom">Personalizado</option></select><div class="universal-schedule-inline" id="usbNotificationCustom"><input type="number" min="1" max="999" id="usbNotificationAmount"><select id="usbNotificationUnit"><option value="minutes">minutos antes</option><option value="hours">horas antes</option><option value="days">dias antes</option></select></div></section></div><input type="hidden" id="usbStart"></div><footer class="universal-schedule-footer"><button class="ghost-btn" type="button" data-usb-close>Cancelar</button><button class="primary-btn" type="button" id="usbApply">Aplicar</button></footer></div>';
    document.body.appendChild(modal);
    modal.addEventListener("click", (event) => { if (event.target.closest("[data-usb-close]")) return closeBridgeModal(); if (event.target.closest("#usbEndButton")) return openRepeatUntilModal(); const repeat = event.target.closest("[data-usb-repeat]"); if (repeat) { const cfg = readTargetSchedule(); cfg.frequency = repeat.dataset.usbRepeat; if (cfg.frequency === "daily") { cfg.intervalUnit="day"; cfg.interval=1; } if (cfg.frequency === "weekly") { cfg.intervalUnit="week"; cfg.interval=1; } if (cfg.frequency === "monthly_custom") { cfg.intervalUnit="month"; cfg.interval=1; } if (cfg.frequency === "yearly") { cfg.intervalUnit="year"; cfg.interval=1; } if (cfg.frequency === "periodic") { cfg.intervalUnit="day"; cfg.interval=Math.max(2,Number(cfg.interval || 2)); cfg.avoidDays=normalizeAvoidDays(cfg.avoidDays,[0,6]); } syncSimpleScheduleFromStartDate(cfg); saveTargetSchedule(cfg); renderBridgeModal(); return; } const avoid = event.target.closest("[data-usb-avoid-day]"); if (avoid) { const cfg = readTargetSchedule(); const selected = new Set(normalizeAvoidDays(cfg.avoidDays,[0,6])); const value = Number(avoid.dataset.usbAvoidDay); selected.has(value) ? selected.delete(value) : selected.size < 6 && selected.add(value); cfg.avoidDays=[...selected].sort((a,b)=>a-b); saveTargetSchedule(cfg); renderBridgeModal(); return; } const day = event.target.closest("[data-usb-weekday]"); if (day) { const cfg = readTargetSchedule(); const selected = new Set(cfg.weekDays || []); const value = Number(day.dataset.usbWeekday); selected.has(value) ? selected.delete(value) : selected.add(value); cfg.weekDays=[...selected].sort((a,b)=>a-b); saveTargetSchedule(cfg); renderBridgeModal(); } });
    modal.querySelector("#usbApply")?.addEventListener("click", applyBridgeModal);
    modal.addEventListener("input", (event) => { if (event.target.matches("#usbInterval")) updateIntervalLabels(modal,modal.querySelector("#usbUnit")?.value || "day",event.target.value); });
    modal.addEventListener("change", (event) => { if (event.target.matches("#usbNotification")) modal.querySelector("#usbNotificationCustom").hidden = event.target.value !== "custom"; if (event.target.matches("#usbUnit")) { const cfg=readTargetSchedule(); cfg.intervalUnit=event.target.value; if (cfg.intervalUnit === "week" && !cfg.weekDays.length) cfg.weekDays=[getMissionLocalWeekday()]; if (cfg.intervalUnit === "month") { const parts=getScheduleDateParts(cfg); cfg.monthDay=parts.day; cfg.monthlyOrdinalIndex=parts.ordinal; cfg.monthlyWeekdayIndex=parts.weekday; } saveTargetSchedule(cfg); renderBridgeModal(); } if (event.target.matches("[name='usbMonthlyMode']")) { const cfg=readTargetSchedule(); cfg.monthlyMode=event.target.value === "day" ? "day" : "weekday"; const parts=getScheduleDateParts(cfg); cfg.monthDay=parts.day; cfg.monthlyOrdinalIndex=parts.ordinal; cfg.monthlyWeekdayIndex=parts.weekday; saveTargetSchedule(cfg); renderBridgeModal(); } });
    return modal;
  }
    ...
    if (!button) {
      ...
      button.addEventListener("click", (event) => {
        ...
        openDailyRepetitionModal("mission-create", () => {
          if (missionCreateStatus) missionCreateStatus.textContent = "Repetição definida.";
          renderMissionCreateStep();
        });
      });
  ...
  function scheduleMatchesToday(config, nowMs = getServerNowMs()) {
    ...
    if (cfg.frequency === "periodic" && normalizeAvoidDays(cfg.avoidDays, []).includes(today.getDay())) return false;
...
// PROJECT200_MICROTASK_FIVE_STEP_FLOW
(function installProject200MicrotaskFiveStepFlow() {
  if (window.__project200MicrotaskFiveStepFlowInstalled) return;
  window.__project200MicrotaskFiveStepFlowInstalled = true;
  ...
  const timeStep = scheduleStage?.closest(".mission-variant-editor-step");
  let repeatStep = document.getElementById("missionVariantEditorStep5");

  function ensureMicrotaskStages() {
    if (!durationStep || !dateStep || !timeStep || durationStep.dataset.microtaskStagesReady === "true") return;
    ...
    timeStep.innerHTML = '<div class="mission-variant-stage-copy"><strong>Horário de início</strong><span>Defina quando esta micro-tarefa fica disponível.</span></div><label class="mission-variant-anytime"><input type="checkbox" id="missionVariantAnytimeInput" checked><span>A qualquer hora do dia</span></label><label class="mission-variant-clock" id="missionVariantClockField" hidden><span>Horário</span><input class="text-field options-text-field" type="time" id="missionVariantTimeInput" value="09:00"></label>';
    ...
    timeStep.querySelector("#missionVariantAnytimeInput")?.addEventListener("change", (event) => {
      state.missionVariants.timeAnytime = Boolean(event.currentTarget.checked);
      renderMissionVariants();
    });
    timeStep.querySelector("#missionVariantTimeInput")?.addEventListener("input", (event) => {
      state.missionVariants.timeValue = String(event.currentTarget.value || "09:00").slice(0, 5);
    });
    durationStep.dataset.microtaskStagesReady = "true";
  }

  function openMicrotaskDateSelector() {
  ...
  function openMicrotaskRepetition() {
  ...
  function renderMicrotaskFlow() {
    ...
    const anytimeInput = document.getElementById("missionVariantAnytimeInput");
    const timeInput = document.getElementById("missionVariantTimeInput");
    const clockField = document.getElementById("missionVariantClockField");
    const anytime = state.missionVariants?.timeAnytime !== false;
    if (anytimeInput) anytimeInput.checked = anytime;
    if (timeInput) timeInput.value = String(state.missionVariants?.timeValue || "09:00").slice(0, 5);
    if (clockField) clockField.hidden = anytime;
    ...
    if (scheduleStage) {
      if (!editorOpen || step !== 5) {
        ...
        scheduleStage.querySelector("#missionVariantUniversalScheduleButton")?.addEventListener("click", openMicrotaskRepetition);
      }
    }
  }
  ...
  renderMissionVariants = function() {
    ...
    if (timeStep) timeStep.hidden = step !== 4;
  ...
  missionVariantEditorSave?.addEventListener("click", async (event) => {
    ...
    if (editorStep === 4) {
      const anytime = document.getElementById("missionVariantAnytimeInput")?.checked !== false;
      const time = String(document.getElementById("missionVariantTimeInput")?.value || state.missionVariants?.timeValue || "09:00").slice(0, 5);
      if (!anytime && !/^[0-9]{2}:[0-9]{2}$/.test(time)) {
        if (missionVariantStatus) missionVariantStatus.textContent = "Defina o horário de início.";
        return;
      }
      state.missionVariants.timeAnytime = anytime;
      state.missionVariants.timeValue = time;
      state.missionVariants.editorStep = 5;
      if (missionVariantStatus) missionVariantStatus.textContent = "";
      renderMissionVariants();
      openMicrotaskRepetition();
      return;
    }
    if (editorStep < 5) {
      state.missionVariants.editorStep = editorStep + 1;
    ...
    const scheduleConfig = {
      ...(state.missionVariants?.scheduleConfig || state.missionVariants?.repeatConfig || {}),
      timeAnytime: state.missionVariants?.timeAnytime !== false,
      time: String(state.missionVariants?.timeValue || "09:00").slice(0, 5)
    };
    const finishLoading = beginMissionActionLoading(missionVariantEditorSave);
    if (!finishLoading) return;
    try {
