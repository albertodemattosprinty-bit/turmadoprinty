...
import { calculateProject200TimedMuscleGains, project200CurrentMuscleProgress, project200DecayedMusclePoints } from "./exercise-muscle-progress.js?v=20260915-performance-v1";
import { exerciseCatalogSearchScore, sortExerciseCatalog } from "./exercise-catalog-search.js?v=20260911-v1";
import { calculateProject200ExerciseMetricGains, defaultProject200ExerciseMetrics, defaultProject200ExerciseMuscleLoads, formatProject200Distance, normalizeProject200ExerciseMetrics, project200ExerciseDurationMinutes, project200MetricColor, project200SeriesWorkSeconds } from "./exercise-performance.js?v=20260915-v1";

const TOKEN_KEY = "turma_do_printy_token";
const PROFILE_KEY = "project_200_profile_v1";
const fatLossExercises = [
  ["walk","Caminhada","gps","Ao ar livre","Mantenha postura alta, passos naturais e ritmo que permita respirar com controle."],
  ["bike-light","Bicicleta leve","gps","Bicicleta comum","Use carga leve, mantenha uma cadência confortável e evite travar os joelhos."],
  ["bike","Bicicleta média","gps","Bicicleta comum","Ajuste o banco, mantenha cadência estável e resistência moderada."],
  ["bike-heavy","Bicicleta pesada","gps","Bicicleta comum","Use resistência alta com postura estável e reduza o ritmo se perder o controle."],
  ["treadmill","Esteira","minutes","Esteira","Comece devagar, olhe para frente e aumente a velocidade apenas quando estiver estável."],
  ["run-light","Corrida leve","gps","Natural","Corra em ritmo confortável, com passos curtos e respiração controlada."],
  ["run","Corrida estável","gps","Natural","Mantenha ritmo contínuo, pouse o pé abaixo do corpo e preserve a postura."],
  ["run-heavy","Corrida pesada","gps","Natural","Sustente um ritmo intenso com técnica; reduza se a passada perder estabilidade."],
...
const EXERCISES = [...strengthExercises, ...fatLossExercises, ...calisthenicsExercises].map((exercise)=>{
  const muscleLoads=defaultProject200ExerciseMuscleLoads(exercise);
  return {...exercise,...defaultProject200ExerciseMetrics(exercise),...(muscleLoads.length?{muscleLoads}: {})};
});

function heuristicExerciseMuscles(exercise){
  const id=String(exercise?.id||"");
  const groups=[
    [/^(walk|treadmill|run(?:-light|-heavy)?|elliptical|stair|rope|swim|dance|hike|jumping-jack|high-knees|shadow-boxing|skating|water-aerobics|spinning|bike(?:-light|-heavy)?)$/, ["Cardio","Pernas"]],
...
const elements = {
  ...
  infoMuscles:byId("wellnessExerciseInfoMuscles"), infoPerformance:byId("wellnessExerciseInfoPerformance"), adminLayer:byId("wellnessExerciseAdminLayer"), adminProgress:byId("wellnessExerciseAdminProgress"), adminProgressFill:byId("wellnessExerciseAdminProgressFill"), adminProgressCount:byId("wellnessExerciseAdminProgressCount"), adminFill:byId("wellnessExerciseAdminFill"), adminNames:byId("wellnessExerciseAdminNames"), adminCreate:byId("wellnessExerciseAdminCreate"), adminStatus:byId("wellnessExerciseAdminStatus"),
  thumbsLayer:byId("wellnessExerciseThumbsLayer"), thumbsProgress:byId("wellnessExerciseThumbProgress"), thumbsProgressFill:byId("wellnessExerciseThumbProgressFill"), thumbsProgressCount:byId("wellnessExerciseThumbProgressCount"), thumbsPrompt:byId("wellnessExerciseThumbPrompt"), thumbsFill:byId("wellnessExerciseThumbFill"), thumbsRegenerate:byId("wellnessExerciseThumbRegenerate"), thumbsStatus:byId("wellnessExerciseThumbStatus"),
  ...
  goalLayer:byId("wellnessGoalLayer"), goalForm:byId("wellnessGoalForm"), goalTitle:byId("wellnessGoalTitle"), goalHelp:byId("wellnessGoalHelp"), goalMetricChoice:byId("wellnessGoalMetricChoice"),
  ...
  phaseUnit:byId("wellnessPhaseUnit"), phaseProgress:byId("wellnessWorkoutProgressRing"), workoutImage:byId("wellnessWorkoutExerciseImage"), workoutVideo:byId("wellnessWorkoutExerciseVideo"), workoutSeriesTimer:byId("wellnessWorkoutSeriesTimer"), workoutSeriesTime:byId("wellnessWorkoutSeriesTime"), workoutSeriesInstruction:byId("wellnessWorkoutSeriesInstruction"), workoutRest:byId("wellnessWorkoutRest"), workoutRestTime:byId("wellnessWorkoutRestTime"), workoutRestAdd:byId("wellnessWorkoutRestAdd"), workoutRestRemove:byId("wellnessWorkoutRestRemove"), workoutGpsReadout:byId("wellnessWorkoutGpsReadout"), workoutGpsValue:byId("wellnessWorkoutGpsValue"), workoutGpsUnit:byId("wellnessWorkoutGpsUnit"), workoutGpsLabel:byId("wellnessWorkoutGpsLabel"), workoutCurrentSpeed:byId("wellnessWorkoutCurrentSpeed"), workoutAverageSpeed:byId("wellnessWorkoutAverageSpeed"), workoutSeriesStatus:byId("wellnessWorkoutSeriesStatus"), workoutSeriesCount:byId("wellnessWorkoutSeriesCount"), workoutPrimary:byId("wellnessWorkoutPrimary"),
...
function activateOfflineWorkout(workout,startPayload){
  ...
  resetGpsDistanceDisplay(state.gpsDistanceMeters);
...
function pauseHiddenExerciseVideos(){ document.querySelectorAll("video[data-exercise-video]").forEach((video)=>{ if(video.closest("[hidden]")){ const shell=video.closest("[data-exercise-video-shell],[data-fixed-exercise-video-shell]");if(shell)showExerciseVideoCover(shell,{reset:true});else video.pause(); } }); }
...
function exerciseCatalog(){
  ...
  exerciseDefinitions().forEach((definition)=>{
    ...
    const current=catalog.get(id)||{},builtIn=Boolean(current.id),fixedMuscleLoads=defaultProject200ExerciseMuscleLoads({id}),metrics=normalizeProject200ExerciseMetrics(definition,{...current,exerciseId:id,category:definition.category||current.category,difficulty:definition.difficulty});
    catalog.set(id,{...current,id,name:(builtIn?current.name:definition.exerciseName)||definition.exerciseName||"Exercício",alternativeNames:Array.isArray(definition.alternativeNames)?definition.alternativeNames:[],names:Array.isArray(definition.names)?definition.names:[],difficulty:Number(definition.difficulty||0),popularity:Number(definition.popularity||0),category:(builtIn?current.category:definition.category)||definition.category||"strength",tracking:(builtIn?current.tracking:definition.trackingType)||definition.trackingType||"series",equipment:definition.equipment||current.equipment||"Sem equipamento",cue:definition.cue||current.cue||"Faça o movimento com controle e respeite seus limites.",muscleLoads:fixedMuscleLoads.length?fixedMuscleLoads:(Array.isArray(definition.muscles)&&definition.muscles.length?definition.muscles:(current.muscleLoads||[])),...metrics});
  });
...
function configureFixedExerciseMedia(shell,image,video,exercise,{movementOnly=false}={}){
  ...
  if(hasVideo){ video.dataset.exerciseVideo=""; if(video.getAttribute("src")!==media.video){video.pause();shell.classList.remove("is-playing");video.setAttribute("src",media.video);video.load();} video.setAttribute("poster",media.poster); video.setAttribute("aria-label",`Vídeo de ${exercise?.name||"exercício"}`);showExerciseVideoCover(shell);return; }
  video.pause(); video.hidden=true; video.removeAttribute("data-exercise-video"); video.removeAttribute("src"); shell.classList.remove("is-playing"); image.removeAttribute("data-exercise-video-cover"); image.dataset.exerciseImageId=String(exercise?.id||""); image.dataset.startSrc=media.cover; image.dataset.finishSrc=media.thumb||media.finish; image.dataset.muscleSrc=media.thumb||media.muscle; if(!movementOnly&&!media.thumb)image.dataset.exerciseImage=""; else image.removeAttribute("data-exercise-image"); image.src=media.cover;
}
...
function startExerciseImageTicker(){ if(!state.exerciseImageTimer)state.exerciseImageTimer=window.setInterval(()=>{ if(document.hidden||!modal?.classList.contains("active")||hasVisibleWellnessPhase())return; const index=EXERCISE_IMAGE_PHASES.indexOf(state.exerciseImagePhase); state.exerciseImagePhase=EXERCISE_IMAGE_PHASES[(index+1)%EXERCISE_IMAGE_PHASES.length]; refreshExerciseImages(); },1500); if(!state.exerciseMuscleTimer)state.exerciseMuscleTimer=window.setInterval(()=>{ if(document.hidden||!modal?.classList.contains("active")||hasVisibleWellnessPhase())return; state.muscleNameIndex+=1; refreshExerciseMuscleLabels(); },1000); if(!state.muscleProgressTimer)state.muscleProgressTimer=window.setInterval(()=>{ if(document.hidden||!modal?.classList.contains("active")||state.tab!=="exercises"||hasVisibleWellnessPhase())return; refreshMuscleProgressMap(); },25000); if(!state.muscleCarouselTimer)state.muscleCarouselTimer=window.setInterval(advanceMuscleProgressCarousel,2000); }
function startDetailMovementTicker(){ if(state.detailImageTimer)window.clearInterval(state.detailImageTimer); state.detailImageTimer=null; state.detailImagePhase="start"; const images=exerciseImageSources(state.selectedExercise); if(images.video){elements.detailVideo.muted=true;showExerciseVideoCover(elements.detailFrame,{reset:true});return;} if(!elements.detailImage)return; elements.detailImage.src=images.cover; if(images.thumb)return; state.detailImageTimer=window.setInterval(()=>{ if(document.hidden||elements.detail?.hidden||exerciseImageSources(state.selectedExercise).video||exerciseImageSources(state.selectedExercise).thumb)return; state.detailImagePhase=state.detailImagePhase==="start"?"finish":"start"; const current=exerciseImageSources(state.selectedExercise); elements.detailImage.src=exerciseImageForPhase(current,state.detailImagePhase); },1000); }
...
function applyLocalMuscleSeriesProgress(workout,repetitions){
  ...
  const gains=calculateProject200TimedMuscleGains(project200SeriesWorkSeconds(repetitions)/60,exerciseMuscleLoads(exercise)); if(!gains.length)return;
...
function hasExerciseGoal(item,exercise=null){ if(!item)return false;const itemTracking=String(item.trackingType||"minutes"),catalogTracking=String(exercise?.tracking||itemTracking);if(catalogTracking==="gps"){if(!["gps","minutes"].includes(itemTracking))return false;}else if(itemTracking!==catalogTracking)return false;if(itemTracking==="series")return Number(item.targetSeries||0)>0&&Number(item.targetReps||0)>0;if(itemTracking==="gps")return Number(item.targetDistanceMeters||0)>0;return Number(item.targetMinutes||0)>0; }
...
function startFirstDailyExercise(){
  ...
  if(state.workout){renderWorkout();showLayer(elements.workoutLayer);return;}
...
function refreshExerciseProgress(){ if(!hasVisibleWellnessPhase())refreshExerciseImages(); renderDailyExerciseProgress(); }
...
function openExerciseDetail(exercise,mode="selected"){
  ...
  if(elements.detailGenerate)elements.detailGenerate.hidden=true;
  ...
  if(elements.detailGenerateStatus&&!isGenerating)elements.detailGenerateStatus.textContent="";
  const item=libraryItem(exercise.id),hasGoal=hasExerciseGoal(item,exercise),selectedTracking=String(item?.trackingType||exercise.tracking),startLabel=mode==="catalog"?"Iniciar exercício":!hasGoal?"Definir nova meta":selectedTracking==="series"?"Iniciar série":selectedTracking==="gps"?"Iniciar percurso":"Iniciar exercício"; elements.detailStart.setAttribute("aria-label",startLabel);elements.detailStart.title=startLabel; if(elements.detailGoal)elements.detailGoal.hidden=mode!=="selected"||!hasGoal; if(elements.detailDelete)elements.detailDelete.hidden=mode!=="selected"||!item;
...
function exerciseHasThumbnail(exercise){return Boolean(String(exerciseAsset(exercise?.id)?.thumbnailUrl||"").trim());}
function renderExerciseThumbAdmin(){
  const catalog=exerciseCatalog(),ready=catalog.filter(exerciseHasThumbnail).length,total=catalog.length,coveragePercent=total?Math.round((ready/total)*100):0,generationPercent=state.thumbGenerationTotal?Math.round((state.thumbGenerationCompleted/state.thumbGenerationTotal)*100):0,percent=state.generatingThumbs?generationPercent:coveragePercent;
  if(elements.thumbsProgress)elements.thumbsProgress.textContent=`${percent}%`;
  if(elements.thumbsProgressFill)elements.thumbsProgressFill.style.width=`${percent}%`;
  if(elements.thumbsProgressCount)elements.thumbsProgressCount.textContent=state.generatingThumbs?`${state.thumbGenerationCompleted} de ${state.thumbGenerationTotal} geradas agora · ${ready} de ${total} com thumb`:`${ready} de ${total} exercícios com thumb`;
  if(elements.thumbsFill){elements.thumbsFill.disabled=state.generatingThumbs||!total;elements.thumbsFill.textContent=state.generatingThumbs?"OpenAI gerando...":ready>=total?"Todas as thumbs estão prontas":"Gerar thumbs pendentes";}
  if(elements.thumbsRegenerate)elements.thumbsRegenerate.disabled=state.generatingThumbs||!total;
  if(elements.thumbsPrompt)elements.thumbsPrompt.disabled=state.generatingThumbs;
}
async function openExerciseThumbAdmin(){
  await refreshAdminAccess();if(!state.isAdmin)return;
  if(!modal?.classList.contains("active"))openWellness("exercises");else setTab("exercises");
  if(!state.dashboard&&navigator.onLine!==false)await loadDashboard({showCached:true});
  state.thumbGenerationCompleted=0;state.thumbGenerationTotal=0;renderExerciseThumbAdmin();showLayer(elements.thumbsLayer);
}
async function generateExerciseThumbs({regenerateAll=false}={}){
  if(!state.isAdmin||state.generatingThumbs)return;
  const catalog=exerciseCatalog(),pending=regenerateAll?catalog:catalog.filter((exercise)=>!exerciseHasThumbnail(exercise));
  if(!pending.length){if(elements.thumbsStatus)elements.thumbsStatus.textContent="Todas as thumbs já estão prontas.";renderExerciseThumbAdmin();return;}
  if(regenerateAll&&!window.confirm(`Gerar novamente as ${pending.length} thumbs? As capas atuais serão substituídas.`))return;
  state.generatingThumbs=true;state.thumbGenerationCompleted=0;state.thumbGenerationTotal=pending.length;renderExerciseThumbAdmin();const failures=[];
  for(const exercise of pending){
    if(elements.thumbsStatus)elements.thumbsStatus.textContent=`Gerando ${state.thumbGenerationCompleted+1} de ${pending.length}: ${exercise.name}...`;
    try{
      const payload=await apiRequest(`/api/admin/200/exercises/${encodeURIComponent(exercise.id)}/thumbnail`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({exerciseName:exercise.name,category:exercise.category,equipment:exercise.equipment,cue:exercise.cue,basePrompt:String(elements.thumbsPrompt?.value||"").trim()}),forceNetwork:true,skipGlobalLoading:true});
      if(!payload?.asset?.thumbnailUrl)throw new Error("A OpenAI não devolveu a thumb pronta.");
      const assets=exerciseAssets().filter((item)=>String(item?.exerciseId||"")!==String(exercise.id));assets.push(payload.asset);state.dashboard={...(state.dashboard||{}),exerciseAssets:assets};cacheDashboard();
    }catch(error){failures.push(`${exercise.name}: ${error instanceof Error?error.message:"falha na geração"}`);}
    state.thumbGenerationCompleted+=1;renderExerciseThumbAdmin();
  }
  state.generatingThumbs=false;renderExerciseThumbAdmin();renderExerciseGrid();if(!elements.catalogLayer?.hidden)renderCatalog();
  if(elements.thumbsStatus)elements.thumbsStatus.textContent=failures.length?`Processo concluído com ${failures.length} falha(s). A próxima execução tentará apenas as pendentes. ${failures[0]}`:`Pronto: ${pending.length} thumb${pending.length===1?"":"s"} 600×600 criada${pending.length===1?"":"s"} com gpt-image-1.`;
}
...
function startCatalogExercise(exercise){if(!exercise)return;if(state.workout){renderWorkout();showLayer(elements.workoutLayer);return;}state.selectedExercise=exercise;state.detailMode="catalog";fillExerciseGoalFields(exercise,null);void startExercise({preventDefault(){},currentTarget:elements.detailStart});}
function exerciseGoalTrackingType(exercise=state.selectedExercise,item=null){const catalogTracking=String(exercise?.tracking||item?.trackingType||"minutes");if(catalogTracking==="gps")return String(item?.trackingType||"")==="minutes"?"minutes":"gps";return catalogTracking;}
function renderExerciseGoalMetricChoice(){
  const exercise=state.selectedExercise,tracking=exerciseGoalTrackingType(exercise,{trackingType:state.goalTrackingType});
  state.goalTrackingType=tracking;
  if(elements.goalMetricChoice){elements.goalMetricChoice.hidden=exercise?.tracking!=="gps";elements.goalMetricChoice.querySelectorAll("[data-goal-tracking]").forEach((button)=>{const active=button.dataset.goalTracking===tracking;button.classList.toggle("active",active);button.setAttribute("aria-pressed",String(active));});}
  const isSeries=tracking==="series",isGps=tracking==="gps";
  elements.goalTitle.textContent=isSeries?"Defina séries e movimentos":isGps?"Qual distância deseja percorrer?":"Quantos minutos?";
  elements.goalStart.textContent=state.goalEditMode?"Salvar meta":isSeries?"Iniciar série":isGps?"Iniciar percurso":"Iniciar exercício";
  elements.goalHelp.textContent=state.goalEditMode?"Ajuste a métrica, a meta e os dias do treino.":"Defina como medir, a meta e os dias antes de iniciar. Tudo realizado será consolidado ao concluir.";
...
function selectExerciseGoalTrackingType(trackingType){if(state.selectedExercise?.tracking!=="gps"||!["minutes","gps"].includes(trackingType))return;state.goalTrackingType=trackingType;renderExerciseGoalMetricChoice();}
function fillExerciseGoalFields(exercise,item){
  state.goalTrackingType=exerciseGoalTrackingType(exercise,item);
  elements.targetSeries.value=String(item?.targetSeries||3);elements.targetReps.value=String(item?.targetReps||12);
  elements.targetMinutes.value=String(item?.targetMinutes||30);elements.targetDistanceKm.value=String((Number(item?.targetDistanceMeters||3000)/1000).toFixed(1));
  renderExerciseGoalMetricChoice();
}
...
function openGoal({editing=false}={}){
  const exercise=state.selectedExercise, item=libraryItem(exercise?.id);
  state.goalEditMode=editing;
  fillExerciseGoalFields(exercise,item); setExerciseGoalWeekdays(item);
  showLayer(elements.goalLayer);
}
function startSelectedExercise(){ const exercise=state.selectedExercise,item=libraryItem(exercise?.id); if(!exercise)return; if(!hasExerciseGoal(item,exercise)){ openGoal({editing:false}); return; } fillExerciseGoalFields(exercise,item); void startExercise({preventDefault(){},currentTarget:elements.detailStart}); }
...
function formatDistance(meters){ return formatProject200Distance(meters); }
function displayedGpsDistance(){return state.gpsDisplayDistanceMeters===null?currentGpsDistance():Math.max(0,Math.round(state.gpsDisplayDistanceMeters));}
function formatGpsSpeed(value){return Math.max(0,Number(value)||0).toLocaleString("pt-BR",{minimumFractionDigits:1,maximumFractionDigits:1});}
function gpsDistanceParts(distance){const formatted=formatDistance(distance),match=formatted.match(/^(.+)\s(m|km)$/);return {value:match?.[1]||formatted,unit:match?.[2]||""};}
function gpsReadoutPresentation(){
  if(state.gpsReadoutMode==="current")return {value:formatGpsSpeed(state.gpsCurrentSpeedKmh),unit:"km/h",label:"Velocidade atual"};
  if(state.gpsReadoutMode==="average")return {value:formatGpsSpeed(averageSpeedKmh()),unit:"km/h",label:"Velocidade média"};
  return {...gpsDistanceParts(displayedGpsDistance()),label:"Distância percorrida"};
}
function renderGpsDistanceReadout(){
  const workout=state.workout;if(workout?.trackingType!=="gps")return;const distance=displayedGpsDistance(),targetMeters=Math.max(100,Number(workout.targetDistanceMeters||100)),presentation=gpsReadoutPresentation();
  if(elements.workoutCounter)elements.workoutCounter.textContent=formatDistance(distance);
  if(elements.workoutGpsReadout){elements.workoutGpsReadout.hidden=false;elements.workoutGpsReadout.setAttribute("aria-label",`${presentation.label}: ${presentation.value} ${presentation.unit}`.trim());}
  if(elements.workoutGpsValue)elements.workoutGpsValue.textContent=presentation.value;
  if(elements.workoutGpsUnit)elements.workoutGpsUnit.textContent=presentation.unit;
  if(elements.workoutGpsLabel)elements.workoutGpsLabel.textContent=presentation.label;
  elements.workoutCurrentSpeed?.classList.toggle("active",state.gpsReadoutMode==="current");elements.workoutAverageSpeed?.classList.toggle("active",state.gpsReadoutMode==="average");
  if(elements.phaseUnit)elements.phaseUnit.textContent="";
  setWorkoutProgress((distance/targetMeters)*100);
}
function showGpsReadout(mode){if(state.workout?.trackingType!=="gps"||!["current","average"].includes(mode))return;if(state.gpsReadoutTimer)window.clearTimeout(state.gpsReadoutTimer);state.gpsReadoutMode=mode;renderGpsDistanceReadout();state.gpsReadoutTimer=window.setTimeout(()=>{state.gpsReadoutTimer=null;state.gpsReadoutMode="distance";renderGpsDistanceReadout();},2000);}
function animateGpsDistance(target=currentGpsDistance()){
  const rounded=Math.max(0,Math.round(Number(target)||0));
  if(state.gpsDisplayDistanceMeters===null||rounded<state.gpsDisplayDistanceMeters){state.gpsDisplayDistanceMeters=rounded;renderGpsDistanceReadout();return;}
  if(state.gpsDisplayAnimationFrame!==null||rounded<=state.gpsDisplayDistanceMeters)return;
  const frame=()=>{state.gpsDisplayAnimationFrame=null;if(state.workout?.trackingType!=="gps")return;const destination=currentGpsDistance();if(state.gpsDisplayDistanceMeters<destination){state.gpsDisplayDistanceMeters+=1;renderGpsDistanceReadout();state.gpsDisplayAnimationFrame=window.requestAnimationFrame(frame);}};
  state.gpsDisplayAnimationFrame=window.requestAnimationFrame(frame);
}
function resetGpsDistanceDisplay(value=null){if(state.gpsDisplayAnimationFrame!==null)window.cancelAnimationFrame(state.gpsDisplayAnimationFrame);if(state.gpsReadoutTimer)window.clearTimeout(state.gpsReadoutTimer);state.gpsDisplayAnimationFrame=null;state.gpsReadoutTimer=null;state.gpsReadoutMode="distance";state.gpsCurrentSpeedKmh=0;state.gpsDisplayDistanceMeters=value===null?null:Math.max(0,Math.round(Number(value)||0));}
...
function renderWorkoutSeriesMarkers(done,target){
  ...
  const totalChanged=Number(elements.workoutSeriesCount.dataset.total||0)!==total;
  if(totalChanged){
    elements.workoutSeriesCount.dataset.total=String(total);
    elements.workoutSeriesCount.innerHTML=Array.from({length:total},(_,index)=>`<span class="wellness-workout-series-marker" data-series-marker="${index+1}" aria-label="Série ${index+1} pendente"><img src="/200/apps/exercicios.png" alt="" /><i aria-hidden="true"><b>${index+1}</b><svg viewBox="0 0 24 24"><path d="m6 12 4 4 8-9"/></svg></i></span>`).join("");
  }
  if(totalChanged||elements.workoutSeriesCount.dataset.done!==String(done)){
    elements.workoutSeriesCount.dataset.done=String(done);
    window.requestAnimationFrame(()=>elements.workoutSeriesCount.querySelectorAll("[data-series-marker]").forEach((marker,index)=>{const complete=index<done;marker.classList.toggle("is-complete",complete);marker.setAttribute("aria-label",`Série ${index+1} ${complete?"concluída":"pendente"}`);}));
  }
  elements.workoutSeriesStatus?.setAttribute("aria-label",`${done} de ${target} séries concluídas`);
}
function seriesWorkDurationSeconds(repetitions){
  return project200SeriesWorkSeconds(repetitions);
}
function stopSeriesWorkTimer(){if(state.seriesWorkTimer)window.clearInterval(state.seriesWorkTimer);state.seriesWorkTimer=null;}
function clearSeriesWork(){
  stopSeriesWorkTimer();state.seriesWorkUntil=0;state.seriesWorkWorkoutId="";state.seriesWorkNumber=0;state.seriesWorkDurationMs=0;
  elements.phaseProgress?.classList.remove("is-series-running");if(elements.workoutSeriesTimer)elements.workoutSeriesTimer.hidden=true;
}
function workoutSeriesRemainingMs(){
  if(!state.workout||state.workout.trackingType!=="series"||String(state.workout.id||"")!==state.seriesWorkWorkoutId||Number(state.workout.seriesCount||0)+1!==state.seriesWorkNumber)return 0;
  return Math.max(0,Number(state.seriesWorkUntil||0)-Date.now());
}
function setWorkoutSeriesAppearance(repetitions,remainingMs){
  const reps=Math.max(1,Math.trunc(Number(repetitions||0)||1)),seconds=Math.max(0,Math.ceil(Number(remainingMs||0)/1000));
  elements.phaseProgress?.classList.add("is-series-running");if(elements.workoutSeriesTimer)elements.workoutSeriesTimer.hidden=false;
  if(elements.workoutSeriesTime)elements.workoutSeriesTime.textContent=`${seconds}s`;
  if(elements.workoutSeriesInstruction)elements.workoutSeriesInstruction.textContent=`Faça ${reps} repetições`;
  elements.phaseProgress?.setAttribute("aria-label",`${seconds} segundos para fazer ${reps} repetições`);
}
function refreshSeriesWork(){
  const reps=currentSeriesReps(),remainingMs=workoutSeriesRemainingMs();setWorkoutSeriesAppearance(reps,remainingMs);
  if(remainingMs<=0)stopSeriesWorkTimer();else if(!state.seriesWorkTimer)state.seriesWorkTimer=window.setInterval(refreshSeriesWork,250);
}
function ensureSeriesWorkTimer(workout,repetitions){
  const done=Number(workout?.seriesCount||0),target=Math.max(1,Number(workout?.targetSeries||1)),workoutId=String(workout?.id||""),seriesNumber=done+1;
  if(workout?.trackingType!=="series"||done>=target||state.workoutStarting||workout?.pendingStart||workoutRestRemainingMs()){clearSeriesWork();return;}
  if(state.seriesWorkWorkoutId!==workoutId||state.seriesWorkNumber!==seriesNumber){
    stopSeriesWorkTimer();state.seriesWorkWorkoutId=workoutId;state.seriesWorkNumber=seriesNumber;state.seriesWorkDurationMs=seriesWorkDurationSeconds(repetitions)*1000;state.seriesWorkUntil=Date.now()+state.seriesWorkDurationMs;
  }
  refreshSeriesWork();
}
function workoutFinishIcon(){return `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 4h8v4a4 4 0 0 1-8 0V4Zm0 2H5v1a4 4 0 0 0 4 4M16 6h3v1a4 4 0 0 1-4 4M12 12v5M8 20h8M9 17h6"/></svg>`;}
function workoutSeriesIcon(){return `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 12 4 4 8-9"/></svg>`;}
function workoutStartIcon(){return `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m9 7 8 5-8 5V7Z"/></svg>`;}
function renderWorkoutFinishAction(action){
  const button=elements.workoutFinish;if(!button)return;const config=action==="finish-series"?{label:"Finalizar série",icon:workoutSeriesIcon()}:action==="start-series"?{label:"Iniciar série",icon:workoutStartIcon()}:{label:"Finalizar exercício",icon:workoutFinishIcon()};
  if(button.dataset.workoutAction!==action){button.dataset.workoutAction=action;button.innerHTML=`${config.icon}<span>${config.label}</span>`;}
...
function setWorkoutRestAppearance(resting,remainingMs=0){
  ...
  if(resting&&elements.workoutSeriesTimer)elements.workoutSeriesTimer.hidden=true;
...
function scheduleSeriesRest(workout=state.workout){
  ...
  clearSeriesRest();state.seriesRestWorkoutId=String(workout.id||"");state.seriesRestPending=false;state.seriesRestUntil=Date.now()+WORKOUT_SERIES_REST_MS;state.seriesRestTimer=window.setInterval(refreshSeriesRest,250);return true;
}
...
function setWorkoutStarting(active){
  state.workoutStarting=Boolean(active);modal?.classList.toggle("is-workout-starting",state.workoutStarting);elements.phaseProgress?.classList.toggle("is-starting",state.workoutStarting);elements.workoutRest?.classList.toggle("is-starting",state.workoutStarting);
  if(state.workoutStarting){stopSeriesWorkTimer();elements.phaseProgress?.classList.remove("is-series-running");if(elements.workoutSeriesTimer)elements.workoutSeriesTimer.hidden=true;}
  [elements.workoutPrimary,elements.workoutAdvance,elements.workoutFinish,elements.workoutClose,elements.workoutPreviousExercise,elements.workoutNextExercise].forEach((button)=>{if(button)button.disabled=state.workoutStarting;});
  if(!state.workoutStarting&&elements.workoutRest){elements.workoutRest.hidden=true;elements.workoutRest.classList.remove("is-starting");}
}
function waitForWorkoutStartTick(milliseconds){return new Promise((resolve)=>window.setTimeout(resolve,milliseconds));}
async function runWorkoutStartCountdown(){
  setWorkoutStarting(true);if(elements.workoutRest)elements.workoutRest.hidden=false;
  for(const label of ["3","2","1"]){if(elements.workoutRestTime)elements.workoutRestTime.textContent=label;await waitForWorkoutStartTick(650);}
  if(elements.workoutRestTime)elements.workoutRestTime.textContent="Vai";await waitForWorkoutStartTick(450);
}
function renderWorkout({syncMedia=true}={}){
  const workout=state.workout; elements.activeWorkout.hidden=!workout; if(!workout){ clearSeriesWork();clearSeriesRest();setWorkoutRestAppearance(false);elements.phaseProgress?.classList.remove("is-gps-workout");if(elements.workoutGpsReadout)elements.workoutGpsReadout.hidden=true;elements.activeWorkoutVideo?.pause(); elements.workoutVideo?.pause();state.workoutNavigationWorkoutId="";state.workoutNavigationExerciseId="";if(elements.workoutNextName)elements.workoutNextName.textContent="—";if(elements.workoutAdvance)elements.workoutAdvance.disabled=true;if(elements.workoutPreviousExercise)elements.workoutPreviousExercise.disabled=true;if(elements.workoutNextExercise)elements.workoutNextExercise.disabled=true; return; }
  if(state.seriesWorkWorkoutId&&String(workout.id||"")!==state.seriesWorkWorkoutId)clearSeriesWork();
  ...
  elements.phaseProgress?.classList.toggle("is-gps-workout",isGps);if(elements.workoutGpsReadout)elements.workoutGpsReadout.hidden=!isGps;
  renderWorkoutNavigation(workout);
  elements.workoutSeriesStatus.hidden=!isSeries;
  elements.workoutName.textContent=workout.exerciseName||"Treino"; elements.workoutCounter.textContent=isSeries?`${Number(workout.seriesCount||0)}/${Number(workout.targetSeries||0)} séries`:isGps?formatDistance(displayedGpsDistance()):formatTimer(elapsedSeconds(workout));
  ...
  if(isSeries){
    ...
    if(remainingMs>0){clearSeriesWork();elements.phaseLabel.textContent="Descanso";elements.workoutVideo?.pause();renderWorkoutFinishAction("start-series");setWorkoutRestAppearance(true,remainingMs);return;}
    if(state.seriesRestUntil)clearSeriesRest();setWorkoutRestAppearance(false);elements.phaseLabel.textContent=done>=target?"Meta concluída":`Série ${done+1} de ${target}`;elements.workoutPrimary.setAttribute("aria-label",`Registrar ${reps} movimentos`);setWorkoutProgress((done/target)*100);
    if(done>=target){clearSeriesWork();renderWorkoutFinishAction("finish-exercise");}else{ensureSeriesWorkTimer(workout,reps);renderWorkoutFinishAction("finish-series");}
    return;
  }
  clearSeriesWork();if(state.seriesRestUntil)clearSeriesRest();setWorkoutRestAppearance(false);renderWorkoutFinishAction("finish-exercise");
  const elapsed=elapsedSeconds(workout);
  if(isGps){ const targetMeters=Math.max(100,Number(workout.targetDistanceMeters||100)); elements.phaseLabel.textContent=`Meta de ${formatDistance(targetMeters)}`;elements.workoutPrimary.hidden=true;animateGpsDistance(currentGpsDistance());renderGpsDistanceReadout(); }
...
function applyDashboardPayload(payload){
  ...
  state.seriesRepsDraft=null; state.steps=Number(state.workout?.steps||0); state.gpsDistanceMeters=Number(state.workout?.distanceMeters||0);resetGpsDistanceDisplay(state.gpsDistanceMeters);
  ...
  if(!elements.thumbsLayer?.hidden)renderExerciseThumbAdmin();
...
function exerciseGoalValues(exercise=state.selectedExercise){
  const trackingType=exercise?.tracking==="gps"&&["minutes","gps"].includes(state.goalTrackingType)?state.goalTrackingType:String(exercise?.tracking||"minutes"),isSeries=trackingType==="series",isGps=trackingType==="gps";
  ...
  return {trackingType,targetSeries:isSeries?targetSeries:0,targetReps:isSeries?targetReps:0,targetMinutes:!isSeries&&!isGps?targetMinutes:0,targetDistanceMeters:isGps?targetDistanceMeters:0};
}
async function saveExerciseGoal(scheduleConfig,{startAfterSave=false}={}){
  ...
  try{ const values=exerciseGoalValues(exercise),payload=await apiRequest("/api/200/exercises/library",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({profile:profileName(),exerciseId:exercise.id,exerciseName:exercise.name,category:exercise.category,equipment:exercise.equipment,...values,scheduleConfig})}); state.dashboard=payload.dashboard||state.dashboard; state.goalEditMode=false; renderExerciseGrid(); if(!elements.catalogLayer?.hidden)renderCatalog(); window.dispatchEvent(new CustomEvent("project200:exercise-mission-updated")); if(startAfterSave){ fillExerciseGoalFields(exercise,libraryItem(exercise.id)); await startExercise({preventDefault(){},currentTarget:elements.detailStart}); }else openExerciseDetail(exercise,"selected"); }
...
async function startExercise(event){
  ...
  const {trackingType,targetSeries,targetReps,targetMinutes,targetDistanceMeters}=exerciseGoalValues(exercise),isSeries=trackingType==="series",isGps=trackingType==="gps";
  ...
  const startPayload={profile:profileName(),exerciseId:exercise.id,exerciseName:exercise.name,category:exercise.category,trackingType,equipment:exercise.equipment,targetSeries:isSeries?targetSeries:0,targetReps:isSeries?targetReps:0,targetMinutes:!isSeries&&!isGps?targetMinutes:0,targetDistanceMeters:isGps?targetDistanceMeters:0};
  const previousDashboard=state.dashboard,preview={...createOfflineWorkout(exercise,startPayload),id:`pending-start-${Date.now()}`,offline:false,pendingStart:true};
  const requestStart=async()=>{if(navigator.onLine===false)return {workout:createOfflineWorkout(exercise,startPayload),offline:true};try{const payload=await apiRequest("/api/200/exercises/start",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(startPayload)});return {workout:payload.workout,dashboard:payload.dashboard,offline:false};}catch(error){if(error?.httpResponse)throw error;return {workout:createOfflineWorkout(exercise,startPayload),offline:true};}};
  try{
    state.workout=preview;state.seriesRepsDraft=null;state.steps=0;state.gpsDistanceMeters=0;resetGpsDistanceDisplay(0);renderWorkout();showLayer(elements.workoutLayer);
    const [started]=await Promise.all([requestStart(),runWorkoutStartCountdown()]);
    if(started.offline)activateOfflineWorkout(started.workout,startPayload);else{state.dashboard=started.dashboard||state.dashboard;state.workout=started.workout;}
    state.seriesSaveChain=Promise.resolve();rebuildProvisionalMuscleProgress(state.workout);state.seriesRepsDraft=null;state.steps=Number(state.workout?.steps||0);state.gpsDistanceMeters=Number(state.workout?.distanceMeters||0);resetGpsDistanceDisplay(state.gpsDistanceMeters);
    if(trackingType==="steps")await startStepCounter(true);if(trackingType==="gps")await startGpsTracking(true);
    setWorkoutStarting(false);renderExerciseGrid();renderWorkout();
  }
  catch(error){state.workout=null;state.dashboard=previousDashboard;setWorkoutStarting(false);const message=error instanceof Error?error.message:"Nao foi possivel iniciar.";if(directStart){openExerciseDetail(exercise,state.detailMode);elements.detailGenerateStatus.textContent=message;}else{showLayer(elements.goalLayer);elements.goalForm.querySelector("p").textContent=message;}}
...
async function handleWorkoutFinishAction(){
  if(!state.workout)return;const action=String(elements.workoutFinish?.dataset.workoutAction||"finish-exercise");
  if(action==="start-series"){skipSeriesRest();return;}
  if(action==="finish-series"){await handleWorkoutPrimary();return;}
  await finishWorkout();
}
...
function workoutCompletionSnapshot(workout){
  const exercise=exerciseCatalog().find((item)=>item.id===workout?.exerciseId)||exerciseFromLibrary(libraryItem(workout?.exerciseId)||{}),library=libraryItem(workout?.exerciseId),projectedWorkout={...workout,distanceMeters:workout?.trackingType==="gps"?currentGpsDistance():Number(workout?.distanceMeters||0),durationMinutes:workout?.trackingType==="series"?project200ExerciseDurationMinutes({...workout,series:workoutSeriesPayload(workout)}):Math.max(Number(workout?.durationMinutes||0),elapsedSeconds(workout)/60)},metrics=normalizeProject200ExerciseMetrics(exercise,exercise),metricGains=calculateProject200ExerciseMetricGains(projectedWorkout,metrics),gains=calculateProject200TimedMuscleGains(metricGains.durationMinutes,exerciseMuscleLoads(exercise)),progress=new Map(),beforeProgress=new Map(),gainByMuscle=new Map(gains.map((gain)=>[gain.muscleId,Number(gain.points||0)]));
  ...
  const media=exerciseImageSources(exercise); return {name:workout?.exerciseName||exercise?.name||"Exercício",exercise,image:media.poster||media.start,maps,progress,beforeProgress,gainByMuscle,metrics,metricGains,metricsBeforeMuscles:workout?.trackingType!=="series",progressFrom,progressTo,progressLabel:planned?"Progresso Total":"Progresso do Exercício",progressMax:planned?100:200};
}
...
const WORKOUT_METRIC_PRESENTATION=Object.freeze([
  {key:"energy",rateKey:"energyPerMinute",title:"+ ENERGIA",label:"Energia",icon:'<path d="M68 7 29 66h28l-6 47 40-64H63L68 7Z"/>'},
  {key:"strength",rateKey:"strengthPerMinute",title:"+ FORÇA",label:"Força",icon:'<path d="M4 45h10V30h10v24h72V30h10v15h10v30h-10v15H96V66H24v24H14V75H4V45Z"/>'},
  {key:"resistance",rateKey:"resistancePerMinute",title:"+ RESISTÊNCIA",label:"Resistência",icon:'<path d="M60 108C26 86 10 67 10 43 10 22 25 10 42 10c9 0 15 4 18 11 4-7 10-11 19-11 17 0 31 12 31 33 0 24-16 43-50 65Zm-31-53h18l7-17 12 37 8-20h17"/>'}
]);
function metricNumber(value){return Number(value||0).toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2});}
async function animateWorkoutMetric(snapshot,descriptor,runId){
  if(state.completionRunId!==runId||!elements.completeMetricStage)return;const total=Number(snapshot.metricGains?.[descriptor.key]||0),rate=Number(snapshot.metrics?.[descriptor.rateKey]||0);elements.completeMetricTitle.textContent=descriptor.title;elements.completeMetricIcon.innerHTML=descriptor.icon;elements.completeMetricIcon.setAttribute("aria-label",descriptor.label);elements.completeMetricRate.textContent=`${metricNumber(rate)} por minuto · ${metricNumber(snapshot.metricGains?.durationMinutes)} min`;elements.completeMetricValue.textContent="+0,00";elements.completeMetricRing.style.setProperty("--metric-progress","0deg");elements.completeMetricRing.style.setProperty("--metric-scale",".72");elements.completeMetricStage.style.setProperty("--metric-color",project200MetricColor(descriptor.key,0));elements.completeMetricStage.hidden=false;elements.completeLayer.classList.remove("is-muscle-stage");elements.completeLayer.classList.add("is-metric-stage");
  await new Promise((resolve)=>{const started=performance.now(),duration=1350;const frame=(now)=>{if(state.completionRunId!==runId){resolve();return;}const ratio=Math.min(1,Math.max(0,(now-started)/duration)),eased=1-((1-ratio)**3),value=total*eased,color=project200MetricColor(descriptor.key,value);elements.completeMetricStage.style.setProperty("--metric-color",color);elements.completeMetricRing.style.setProperty("--metric-progress",`${Math.min(360,(value/80)*360)}deg`);elements.completeMetricRing.style.setProperty("--metric-scale",String(.72+(.28*Math.min(1,value/80))));elements.completeMetricValue.textContent=`+${metricNumber(value)}`;if(ratio<1)window.requestAnimationFrame(frame);else resolve();};window.requestAnimationFrame(frame);});
  await waitForCompletionTime(280);elements.completeMetricStage.hidden=true;elements.completeLayer.classList.remove("is-metric-stage");
}
async function animateWorkoutMetrics(snapshot,runId){for(const descriptor of WORKOUT_METRIC_PRESENTATION){if(state.completionRunId!==runId)return;await animateWorkoutMetric(snapshot,descriptor,runId);}}
async function animateWorkoutMuscles(snapshot,runId){
  elements.completeLayer.classList.remove("is-metric-stage");elements.completeLayer.classList.add("is-muscle-stage");
  if(!snapshot.maps.length){await waitForCompletionTime(420);return;}
...
async function startExerciseAfterCompletion(exercise){
  ...
  if(!hasExerciseGoal(item,exercise)){openGoal({editing:false});return false;}
...
async function finishWorkout({nextExercise=null}={}){
  ...
  if(!nextExercise)void warmWorkoutCompletionSound();clearSeriesWork();clearSeriesRest();
  ...
  try{
    const distanceMeters=workout.trackingType==="gps"?currentGpsDistance():Math.max(0,Number(workout.distanceMeters||0)),series=workoutSeriesPayload(workout),completed={...workout,steps:state.steps,distanceMeters,durationMinutes:completion.metricGains.durationMinutes,energyPoints:completion.metricGains.energy,strengthPoints:completion.metricGains.strength,resistancePoints:completion.metricGains.resistance,completedAt:new Date().toISOString(),status:"completed"};
    ...
    if(String(state.workout?.id||"")===String(workout.id||""))state.workout=null;state.steps=0;state.gpsDistanceMeters=0;resetGpsDistanceDisplay(null);state.provisionalMuscleProgress.clear();cacheDashboard();renderExerciseGrid();renderWorkoutHistory();void stopGpsTracking();stopStepCounter();
  ...
  }catch(error){state.completionRunId+=1;state.dashboard=dashboardBefore;state.workout=workout;state.steps=stepsBefore;state.gpsDistanceMeters=gpsDistanceBefore;resetGpsDistanceDisplay(gpsDistanceBefore);rebuildProvisionalMuscleProgress(workout);cacheDashboard();renderExerciseGrid();renderWorkoutHistory();renderWorkout();if(workout.trackingType==="gps")void startGpsTracking(false);if(workout.trackingType==="steps")void startStepCounter(false);showLayer(elements.workoutLayer);window.alert(error instanceof Error?error.message:"Nao foi possivel encerrar.");}
  finally{submit.disabled=false;}
}
async function discardWorkout(){ if(!state.workout)return; elements.discardWorkout.disabled=true; try{ if(isOfflineWorkout())removeOfflineWorkout(state.workout.id); else await apiRequest(`/api/200/exercises/${encodeURIComponent(state.workout.id)}`,{method:"DELETE"}); await stopGpsTracking(); stopStepCounter(); clearSeriesWork();clearSeriesRest();state.workout=null; state.steps=0; state.gpsDistanceMeters=0;resetGpsDistanceDisplay(null); state.gpsLastPoint=null; state.provisionalMuscleProgress.clear(); state.dashboard={...(state.dashboard||{}),activeWorkout:null}; cacheDashboard(); hideLayers(); if(navigator.onLine!==false)await loadDashboard(); else{renderExerciseGrid();renderWorkoutHistory();renderWorkout();} }catch(error){ elements.finishQuestion.textContent=error instanceof Error?error.message:"Nao foi possivel excluir."; }finally{ elements.discardWorkout.disabled=false; } }
function handleGpsPosition(position,error){ if(error){ state.gpsCurrentSpeedKmh=0;state.gpsStatus="GPS sem sinal"; renderWorkout(); return; } const coords=position?.coords; if(!coords)return; const accuracy=Math.max(0,Number(coords.accuracy||0)),sensorSpeed=Number.isFinite(Number(coords.speed))&&Number(coords.speed)>=0?Number(coords.speed):null,point={latitude:Number(coords.latitude),longitude:Number(coords.longitude),accuracy,timestamp:Number(position.timestamp||Date.now())}; if(!Number.isFinite(point.latitude)||!Number.isFinite(point.longitude))return; if(accuracy>50){ state.gpsCurrentSpeedKmh=0;state.gpsAccuracy=accuracy; state.gpsStatus=`Precisão baixa (${Math.round(accuracy)} m)`; renderWorkout(); return; } if(!state.gpsLastPoint){ state.gpsLastPoint=point;state.gpsCurrentSpeedKmh=Math.min(120,(sensorSpeed||0)*3.6); state.gpsAccuracy=accuracy; state.gpsStatus="GPS ultra preciso ativo"; renderWorkout(); return; } const segment=haversineMeters(state.gpsLastPoint,point), seconds=Math.max(.25,(point.timestamp-state.gpsLastPoint.timestamp)/1000), speed=segment/seconds, minimum=Math.max(1,Math.min(6,(state.gpsLastPoint.accuracy+accuracy)/6)),accepted=segment>=minimum&&segment<=300&&speed<=18;state.gpsCurrentSpeedKmh=Math.min(120,Math.max(0,(sensorSpeed!==null?sensorSpeed:(accepted?speed:0))*3.6)); if(accepted){ state.gpsDistanceMeters+=segment; state.gpsLastPoint=point;animateGpsDistance(state.gpsDistanceMeters); if(Math.round(state.gpsDistanceMeters)%25<Math.round(segment))void saveWorkoutProgress().catch(()=>{}); }else if(segment>300||speed>18){ state.gpsLastPoint=point; } state.gpsAccuracy=accuracy; state.gpsStatus=`GPS ativo · precisão ${Math.round(accuracy)} m`; renderWorkout(); }
async function startGpsTracking(requestPermission){ if(state.gpsWatchId!==null||state.workout?.trackingType!=="gps")return; state.gpsDistanceMeters=Math.max(state.gpsDistanceMeters,Number(state.workout.distanceMeters||0));if(state.gpsDisplayDistanceMeters===null)resetGpsDistanceDisplay(state.gpsDistanceMeters); state.gpsStatus="Buscando GPS preciso..."; renderWorkout(); const plugin=state.gpsPlugin||(window.Capacitor?.isNativePlatform?.()&&window.Capacitor?.registerPlugin?window.Capacitor.registerPlugin("Geolocation"):null); state.gpsPlugin=plugin||null; try{ if(plugin){ if(requestPermission){ const permission=await plugin.requestPermissions({permissions:["location"]}); if(permission?.location==="denied"){ state.gpsStatus="Permita a localização precisa para continuar"; renderWorkout(); return; } } state.gpsProvider="capacitor"; state.gpsWatchId=await plugin.watchPosition({enableHighAccuracy:true,timeout:15000,maximumAge:0,minimumUpdateInterval:1000,interval:1000},handleGpsPosition); }else if(navigator.geolocation){ state.gpsProvider="browser"; state.gpsWatchId=navigator.geolocation.watchPosition((position)=>handleGpsPosition(position), (error)=>handleGpsPosition(null,error), {enableHighAccuracy:true,timeout:15000,maximumAge:0}); }else{ throw new Error("GPS indisponível neste aparelho."); } if(state.saveTimer)window.clearInterval(state.saveTimer); state.saveTimer=window.setInterval(()=>void saveWorkoutProgress().catch(()=>{}),20000); }catch(error){ state.gpsWatchId=null; state.gpsStatus=error instanceof Error?error.message:"Não foi possível iniciar o GPS."; renderWorkout(); } }
async function stopGpsTracking(){ const watchId=state.gpsWatchId, provider=state.gpsProvider; state.gpsWatchId=null; state.gpsProvider=""; state.gpsLastPoint=null;state.gpsCurrentSpeedKmh=0;if(state.gpsReadoutTimer)window.clearTimeout(state.gpsReadoutTimer);state.gpsReadoutTimer=null;state.gpsReadoutMode="distance"; if(watchId!==null){ try{ if(provider==="capacitor")await state.gpsPlugin?.clearWatch({id:String(watchId)}); else if(provider==="browser")navigator.geolocation?.clearWatch(watchId); }catch{} } if(state.saveTimer)window.clearInterval(state.saveTimer); state.saveTimer=null; }
...
byId("wellnessExerciseThumbsClose")?.addEventListener("click",hideLayers);
elements.thumbsFill?.addEventListener("click",()=>void generateExerciseThumbs());
elements.thumbsRegenerate?.addEventListener("click",()=>void generateExerciseThumbs({regenerateAll:true}));
...
elements.phaseProgress?.addEventListener("click",(event)=>{if(event.target.closest("button")||workoutRestRemainingMs()||state.workoutStarting)return;toggleExerciseVideo(elements.phaseProgress);});
byId("wellnessGoalClose")?.addEventListener("click",()=>openExerciseDetail(state.selectedExercise,state.detailMode));
elements.goalForm?.addEventListener("click",(event)=>{ const tracking=event.target.closest("[data-goal-tracking]");if(tracking){selectExerciseGoalTrackingType(tracking.dataset.goalTracking);return;}const weekday=event.target.closest("[data-exercise-weekday]"); if(weekday){ toggleExerciseGoalWeekday(weekday.dataset.exerciseWeekday); return; } const adjust=event.target.closest("[data-goal-adjust]"); if(adjust)adjustExerciseGoalValue(adjust.dataset.goalAdjust,adjust.dataset.direction); });
...
elements.workoutCurrentSpeed?.addEventListener("click",()=>showGpsReadout("current"));
elements.workoutAverageSpeed?.addEventListener("click",()=>showGpsReadout("average"));
...
elements.workoutFinish?.addEventListener("click",()=>void handleWorkoutFinishAction());
...
modal?.addEventListener("pointerdown",()=>{if(state.tab==="exercises"){void warmExerciseProgressSound();void warmWorkoutCompletionSound();}},{passive:true});
  state.ticker=window.setInterval(()=>{
    ...
    if(!state.workout||state.workoutStarting)return;
    renderWorkout({syncMedia:false});
    refreshExerciseProgress();
    if(!elements.detail?.hidden&&state.detailMode==="selected")updateExerciseProgressDetail();
  },2000);
window.addEventListener("pagehide",()=>{ clearSeriesWork();clearSeriesRest();if(state.workout)void saveWorkoutProgress().catch(()=>{}); });
window.addEventListener("online",()=>void syncOfflineWorkouts());
document.addEventListener("visibilitychange",()=>{ if(document.visibilityState!=="visible")return;if(state.seriesRestUntil)refreshSeriesRest();else if(state.seriesWorkUntil)refreshSeriesWork();if(navigator.onLine!==false)void syncOfflineWorkouts(); });
...
window.project200Wellness={open:openWellness,close:closeWellness,openThumbsAdmin:openExerciseThumbAdmin,get exercises(){return exerciseCatalog();}};
