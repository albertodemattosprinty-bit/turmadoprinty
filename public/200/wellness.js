...
function hasExerciseGoal(item,exercise=null){ if(!item)return false;const tracking=String(exercise?.tracking||item.trackingType||"minutes");if(String(item.trackingType||tracking)!==tracking)return false; if(tracking==="series")return Number(item.targetSeries||0)>0&&Number(item.targetReps||0)>0; if(tracking==="gps")return Number(item.targetDistanceMeters||0)>0; return Number(item.targetMinutes||0)>0; }
...
function openExerciseDetail(exercise,mode="selected"){
  ...
  const item=libraryItem(exercise.id),hasGoal=hasExerciseGoal(item,exercise),startLabel=mode==="catalog"?"Iniciar exercício":!hasGoal?"Definir nova meta":exercise.tracking==="series"?"Iniciar série":exercise.tracking==="gps"?"Iniciar percurso":"Iniciar exercício"; elements.detailStart.setAttribute("aria-label",startLabel);elements.detailStart.title=startLabel; if(elements.detailGoal)elements.detailGoal.hidden=mode!=="selected"||!hasGoal; if(elements.detailDelete)elements.detailDelete.hidden=mode!=="selected"||!item;
...
function renderGpsDistanceReadout(){
  const workout=state.workout;if(workout?.trackingType!=="gps")return;const distance=displayedGpsDistance(),targetMeters=Math.max(100,Number(workout.targetDistanceMeters||100)),elapsed=elapsedSeconds(workout);
  if(elements.workoutCounter)elements.workoutCounter.textContent=formatDistance(distance);
  if(elements.phaseUnit)elements.phaseUnit.textContent=`${formatDistance(distance)} · ${formatTimer(elapsed)} · média ${formatAverageSpeed()} · ${state.gpsStatus}`;
...
function resetGpsDistanceDisplay(value=null){if(state.gpsDisplayAnimationFrame!==null)window.cancelAnimationFrame(state.gpsDisplayAnimationFrame);state.gpsDisplayAnimationFrame=null;state.gpsDisplayDistanceMeters=value===null?null:Math.max(0,Math.round(Number(value)||0));}
...
function renderWorkout({syncMedia=true}={}){
  ...
  if(isGps){ const targetMeters=Math.max(100,Number(workout.targetDistanceMeters||100)); elements.phaseLabel.textContent=`Meta de ${formatDistance(targetMeters)}`; elements.workoutPrimary.setAttribute("aria-label","Salvar progresso do GPS");animateGpsDistance(currentGpsDistance());renderGpsDistanceReadout(); }
...
async function startExercise(event){
  ...
  try{
    ...
    if(exercise.tracking==="steps")await startStepCounter(true);if(exercise.tracking==="gps")await startGpsTracking(true);
...
function handleGpsPosition(position,error){ if(error){ state.gpsStatus="GPS sem sinal"; renderWorkout(); return; } const coords=position?.coords; if(!coords)return; const accuracy=Math.max(0,Number(coords.accuracy||0)), point={latitude:Number(coords.latitude),longitude:Number(coords.longitude),accuracy,timestamp:Number(position.timestamp||Date.now())}; if(!Number.isFinite(point.latitude)||!Number.isFinite(point.longitude))return; if(accuracy>50){ state.gpsAccuracy=accuracy; state.gpsStatus=`Precisão baixa (${Math.round(accuracy)} m)`; renderWorkout(); return; } if(!state.gpsLastPoint){ state.gpsLastPoint=point; state.gpsAccuracy=accuracy; state.gpsStatus="GPS ultra preciso ativo"; renderWorkout(); return; } const segment=haversineMeters(state.gpsLastPoint,point), seconds=Math.max(.25,(point.timestamp-state.gpsLastPoint.timestamp)/1000), speed=segment/seconds, minimum=Math.max(1,Math.min(6,(state.gpsLastPoint.accuracy+accuracy)/6)); if(segment>=minimum&&segment<=300&&speed<=18){ state.gpsDistanceMeters+=segment; state.gpsLastPoint=point;animateGpsDistance(state.gpsDistanceMeters); if(Math.round(state.gpsDistanceMeters)%25<Math.round(segment))void saveWorkoutProgress().catch(()=>{}); }else if(segment>300||speed>18){ state.gpsLastPoint=point; } state.gpsAccuracy=accuracy; state.gpsStatus=`GPS ativo · precisão ${Math.round(accuracy)} m`; renderWorkout(); }
