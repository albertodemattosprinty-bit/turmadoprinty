...
import { calculateProject200ExerciseMuscleGains, project200CurrentMuscleProgress } from "./exercise-muscle-progress.js?v=20260910-overload-v1";
...
function formatMuscleLoad(load){ return isMuscleLoadDefined(load)?Number(load).toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2}):""; }
...
async function finishWorkout(event){ event.preventDefault(); if(!state.workout)return; const submit=elements.finishForm.querySelector("button[type=submit]"),workout=state.workout,completion=workoutCompletionSnapshot(workout),completionRunId=prepareWorkoutCompletion(completion); submit.disabled=true; try{ await saveWorkoutProgress(); const distanceMeters=workout.trackingType==="gps"?currentGpsDistance():workout.trackingType==="series"?0:Math.max(0,Math.trunc(Number(elements.distanceInput.value||0)||0)),series=workoutSeriesPayload(workout); if(isOfflineWorkout(workout)){ const completed={...workout,steps:state.steps,distanceMeters,completedAt:new Date().toISOString(),status:"completed"}; persistOfflineWorkout(completed,{finished:true,finishPayload:{steps:state.steps,distanceMeters,series}}); state.dashboard={...(state.dashboard||{}),activeWorkout:null,recentWorkouts:[completed,...(state.dashboard?.recentWorkouts||[])]}; cacheDashboard(); }else{ await apiRequest(`/api/200/exercises/${encodeURIComponent(workout.id)}/finish`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({steps:state.steps,distanceMeters,series}),offlineInvalidates:["/api/200/wellness"],skipGlobalLoading:true}); } await stopGpsTracking(); stopStepCounter(); if(String(state.workout?.id||"")===String(workout.id||""))state.workout=null; state.steps=0; state.gpsDistanceMeters=0; state.provisionalMuscleProgress.clear(); state.dashboard={...(state.dashboard||{}),activeWorkout:null}; applyCompletedWorkoutToLibrary(workout); cacheDashboard(); void animateWorkoutCompletion(completion,completionRunId); if(navigator.onLine!==false)void loadDashboard({showCached:false}); else{renderExerciseGrid();renderWorkoutHistory();renderWorkout();window.dispatchEvent(new CustomEvent("project200:exercise-mission-updated"));} }catch(error){ state.completionRunId+=1; showLayer(elements.finishLayer); elements.finishQuestion.textContent=error instanceof Error?error.message:"Nao foi possivel encerrar."; }finally{ submit.disabled=false; } }
