...
function refreshMuscleProgressMap(){
  ...
  elements.muscleProgressMapTrack.querySelectorAll("[data-current-muscle-id]").forEach((region)=>{
    const muscleId=region.dataset.currentMuscleId,item=byMuscle.get(muscleId),muscle=PROJECT200_MUSCLE_BY_ID[muscleId],points=Number(item?.points||0),percent=Number(item?.percent||0),opacity=points?Math.min(1,.42+(Math.min(percent,110)/110)*.58):.42;
    region.classList.toggle("is-trained",points>0); region.style.setProperty("--muscle-color",item?.color||"rgb(148 163 184)"); region.style.setProperty("--muscle-opacity",String(opacity)); region.setAttribute("aria-label",`${muscle?.name||muscleId} · ${formatMusclePoints(points)} pontos · ${formatMusclePercent(percent)}% · ${item?.stage||"Irrelevante"}`);
  });
