...
function getExploreAuthPrompt() {
  return isAndroidApp
    ? "Debug, sua IA para conversar e criar."
    : "Entre ou cadastre sua conta para abrir o chat.";
}

function getExploreAuthLockMessage() {
  return isAndroidApp
    ? "Entre para acessar o Debug."
    : "Entre para liberar a conversa.";
}

const sessionStorageKey = "turma_do_printy_token";
const conversationStorageKey = "turma_do_printy_debug_conversations";
const folderStorageKey = "turma_do_printy_debug_folders";
const legacyHistoryStorageKey = "turma_do_printy_debug_history";
const chatModeStorageKey = "turma_do_printy_debug_mode";
const chatSettingsStorageKey = "turma_do_printy_debug_settings";
...
const emptyConversationSupportText = "Posso ajudar com ideias, textos, organizacao, planejamento, estudos, codigo e conversas em geral.";
...
function fillSettingsForm() {
  ...
  settingsRoleSelect.value = settings.ministryRole || "Criador";
...
function buildResponseStylePrompt(modeKey) {
  const settings = getUserChatSettings();
  const parts = [
    "Responda de modo respeitoso, aberto e 100% neutro, sem inserir religiao por conta propria.",
  ...
  if (settings.ministryRole) {
    parts.push(`Considere que o perfil da pessoa e ${settings.ministryRole}.`);
  }

  if (settings.ministryDream) {
    parts.push(`Foco atual da pessoa: ${settings.ministryDream}.`);
  }
  ...
  if (modeKey === "project") {
    parts.push("Modo atual: Projeto. Este e o modo mais forte para trabalhar ideias em contexto, planejar melhor e aproveitar memoria expandida.");
...
settingsForm?.addEventListener("submit", (event) => {
  ...
  saveUserChatSettings({
    ...
    ministryRole: settingsRoleSelect.value || "Criador"
  });
