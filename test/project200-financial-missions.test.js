...
db.query = async (sql, params = []) => {
  ...
  if (sql.includes("from project200_financial_goals goal")) {
    return { rows: params[0] === "owner" && params[1] === "Lucas" ? [{ id: goalId, assigned_profile: "Lucas", name: "Reserva", target_amount_cents: 1000, progress_cents: 100, start_on: postgresDate("2026-09-17"), target_on: postgresDate("2026-10-02"), gradual_model: true, initial_deposit_cents: 100, status: "ACTIVE" }] : [], rowCount: 1 };
  }
  if (sql.startsWith("insert into project200_financial_goals")) {
    return { rows: [{ id: goalId, assigned_profile: params[1], name: params[2], target_amount_cents: params[3], start_on: postgresDate(params[4]), target_on: postgresDate(params[5]), gradual_model: params[6], initial_deposit_cents: params[7], progress_cents: 0, status: "ACTIVE" }], rowCount: 1 };
  }
...
test("existing financial goals appear once in every mission scope and preserve cumulative progress", async () => {
  for (const scope of ["today", "last7", "last15", "last30"]) {
    ...
    assert.equal(result.goals[0].financialGoal.startOn, "2026-09-17");
    assert.equal(result.goals[0].financialGoal.targetOn, "2026-10-02");
...
test("creating a gradual goal persists the chosen first deposit without recording fictitious progress", async () => {
  ...
  assert.equal(goal.startOn, "2026-09-17");
  assert.equal(goal.targetOn, "2026-10-02");
