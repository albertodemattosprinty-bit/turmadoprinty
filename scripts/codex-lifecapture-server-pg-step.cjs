...
if (!s.includes('async function handleProject200LifeCaptureUploadRequest(request, response) {')) throw new Error('upload handler not found');
if (!s.includes('async function ensureProject200LifeCapturesSchema() {')) {
  const helper = `async function ensureProject200LifeCapturesSchema() {
  await query(\`
...
`;
  s = s.replace('async function handleProject200LifeCaptureUploadRequest(request, response) {', helper + 'async function handleProject200LifeCaptureUploadRequest(request, response) {');
}
s = s.replace(/const kind = String\(body\?\.kind \|\| "photo"\)\.trim\(\)\.toLowerCase\(\) === "video" \? "video" : "photo";\n\s+const mimeType = String\(body\?\.mimeType \|\| ""\)\.trim\(\)\.toLowerCase\(\);\n\s+const fileBase64 = String\(body\?\.fileBase64 \|\| ""\)\.trim\(\);\n\s+const previewBase64 = String\(body\?\.previewBase64 \|\| ""\)\.trim\(\);/, `const kind = String(body?.kind || "photo").trim().toLowerCase() === "video" ? "video" : "photo";\n    const captureId = String(body?.captureId || body?.id || "").trim() || crypto.randomUUID();\n    const title = String(body?.title || "").trim();\n    const noteText = String(body?.noteText || "").trim();\n    const createdAt = body?.createdAt;\n    const durationMs = Number(body?.durationMs || 0);\n    const metadata = body?.metadata && typeof body.metadata === "object" ? body.metadata : {};\n    const mimeType = String(body?.mimeType || "").trim().toLowerCase();\n    const fileBase64 = String(body?.fileBase64 || "").trim();\n    const previewBase64 = String(body?.previewBase64 || "").trim();`);
s = s.replace(/sendJson\(response, 201, \{\n\s+ok: true,\n\s+asset: \{\n\s+kind,\n\s+key: mediaKey,\n\s+url: buildPublicR2UrlFromKey\(mediaKey\),\n\s+previewKey,\n\s+previewUrl,\n\s+sizeBytes: mediaBuffer.length,\n\s+mimeType\n\s+\}\n\s+\}\);/, `const capture = await upsertProject200LifeCaptureRecord(user.id, {\n      id: captureId,\n      kind,\n      title,\n      noteText,\n      createdAt,\n      mimeType,\n      mediaKey,\n      mediaUrl: buildPublicR2UrlFromKey(mediaKey),\n      previewKey,\n      previewUrl,\n      sizeBytes: mediaBuffer.length,\n      durationMs,\n      metadata\n    });\n\n    sendJson(response, 201, {\n      ok: true,\n      asset: {\n        kind,\n        key: mediaKey,\n        url: buildPublicR2UrlFromKey(mediaKey),\n        previewKey,\n        previewUrl,\n        sizeBytes: mediaBuffer.length,\n        mimeType\n      },\n      capture\n    });`);
if (!s.includes('if (request.method === "GET" && pathname === "/api/200/life-captures") {')) {
  s = s.replace(`  if (request.method === "POST" && pathname === "/api/200/life-captures/upload") {
