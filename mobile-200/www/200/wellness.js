...
import { calculateProject200ExerciseMuscleGains, project200CurrentMuscleProgress } from "./exercise-muscle-progress.js";
...
function currentMuscleProgress(){
  return (Array.isArray(state.dashboard?.muscleProgress)?state.dashboard.muscleProgress:[])
    .map((item)=>project200CurrentMuscleProgress(item))
    .filter((item)=>item.muscleId)
    .sort((left,right)=>right.points-left.points);
}
function applyLocalMuscleProgress(workout){
  ...
  const gains=calculateProject200ExerciseMuscleGains(workout.series,exerciseMuscleLoads(exercise)); if(!gains.length)return;
  const current=new Map(currentMuscleProgress().map((item)=>[item.muscleId,item.points]));
  gains.forEach((item)=>current.set(item.muscleId,Number(current.get(item.muscleId)||0)+Number(item.points||0)));
  state.dashboard={...(state.dashboard||{}),muscleProgress:[...current.entries()].map(([muscleId,points])=>({muscleId,points,updatedAt:now,measuredAt:now}))};
}
...
function refreshMuscleProgressMap(){
  ...
  elements.muscleProgressMapTrack.querySelectorAll("[data-current-muscle-id]").forEach((region)=>{
    const muscleId=region.dataset.currentMuscleId,item=byMuscle.get(muscleId),muscle=PROJECT200_MUSCLE_BY_ID[muscleId],points=Number(item?.points||0),percent=Number(item?.percent||0),opacity=points?Math.min(1,.22+(Math.min(percent,110)/110)*.78):.1;
    region.classList.toggle("is-trained",points>0); region.style.setProperty("--muscle-color",item?.color||"rgb(255 255 255)"); region.style.setProperty("--muscle-opacity",String(opacity)); region.setAttribute("aria-label",`${muscle?.name||muscleId} · ${formatMusclePoints(points)} pontos · ${formatMusclePercent(percent)}% · ${item?.stage||"Irrelevante"}`);
  });
  const strongest=progress[0],muscle=PROJECT200_MUSCLE_BY_ID[strongest?.muscleId];
  if(elements.muscleProgressSummary)elements.muscleProgressSummary.textContent=strongest&&strongest.points>0?`${muscle?.name||"Maior carga"} · ${formatMusclePoints(strongest.points)} pts · ${formatMusclePercent(strongest.percent)}% · ${strongest.stage}`:"Nenhum músculo pontuado";
}
function renderMuscleProgressMap(){
  if(!elements.muscleProgressMapTrack)return;
  elements.muscleProgressMapTrack.innerHTML=PROJECT200_MUSCLE_MAPS.map((map,index)=>`<section class="wellness-muscle-map-slide" data-muscle-progress-map-slide="${index}" aria-label="${escapeHtml(map.title)}"><span>${escapeHtml(map.title)}</span><svg viewBox="${escapeHtml(map.viewBox)}" role="img" aria-label="${escapeHtml(map.title)}"><path class="wellness-muscle-map-skin" d="${escapeHtml(map.skinPath)}"></path>${map.regions.map((region)=>`<path class="wellness-muscle-map-region wellness-muscle-progress-region" data-muscle-id="${escapeHtml(region.muscleId)}" data-current-muscle-id="${escapeHtml(region.muscleId)}" role="button" tabindex="0" d="${escapeHtml(region.path)}"></path>`).join("")}</svg></section>`).join("");
