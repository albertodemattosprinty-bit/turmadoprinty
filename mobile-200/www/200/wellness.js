...
function renderExerciseGrid(){
  ...
  const visible=exerciseLibrary().filter((item)=>category.id==="all"||item.category===category.id).map(exerciseFromLibrary).filter(Boolean);
...
function renderCatalog(){
  ...
  const visible=EXERCISES.filter((item)=>(category.id==="all"||item.category===category.id)&&!selectedIds.has(item.id));
...
function exerciseGoalDateKey(){ const date=new Date(); return `${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,"0")}-${String(date.getDate()).padStart(2,"0")}`; }
