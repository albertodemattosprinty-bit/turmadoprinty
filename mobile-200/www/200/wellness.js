...
const elements = {
  ...
  finishQuestion:byId("wellnessFinishQuestion"), discardWorkout:byId("wellnessDiscardWorkout"), distanceField:byId("wellnessDistanceField"), distanceInput:byId("wellnessDistanceInput"), completeLayer:byId("wellnessWorkoutCompleteLayer"), completeName:byId("wellnessWorkoutCompleteName"), completeImage:byId("wellnessWorkoutCompleteImage"), completeMap:byId("wellnessWorkoutCompleteMap"), completeDots:byId("wellnessWorkoutCompleteDots"), weightCard:byId("wellnessWeightCard"),
...
async function deleteWorkoutHistoryEntry(workout){
  ...
  if(isOfflineWorkout(workout)){ removeOfflineWorkout(workout.id);applyWorkoutRemovalLocally(workout);return; }
...
function renderWorkout(){
  ...
  if(isSeries){ const done=Number(workout.seriesCount||0), target=Math.max(1,Number(workout.targetSeries||1)), reps=currentSeriesReps(); elements.phaseLabel.textContent=done>=target?"Meta concluída · série extra":`Série ${done+1} de ${target}`; renderWorkoutSeriesMarkers(done,target); elements.phaseNumber.textContent=String(reps); elements.phaseUnit.textContent=""; elements.workoutPrimaryLabel.textContent=`Adicionar ${reps}`; setWorkoutProgress((done/target)*100); return; }
...
function workoutCompletionSnapshot(workout){
  ...
  const media=exerciseImageSources(exercise); return {name:workout?.exerciseName||exercise?.name||"Exercício",image:media.poster||media.start,maps,progress};
}
...
function prepareWorkoutCompletion(snapshot){
  const runId=++state.completionRunId; elements.completeName.textContent=snapshot.name; elements.completeImage.src=snapshot.image; elements.completeImage.alt=`${snapshot.name} concluído`; elements.completeDots.innerHTML=snapshot.maps.map((_,index)=>`<i class="${index===0?"is-active":""}"></i>`).join(""); elements.completeMap.classList.remove("is-revealed"); elements.completeMap.innerHTML=snapshot.maps[0]?workoutCompletionMapMarkup(snapshot,snapshot.maps[0]):""; showLayer(elements.completeLayer); return runId;
}
...
async function animateWorkoutCompletion(snapshot,runId){
  if(!snapshot.maps.length){ await waitForCompletionTime(1200); if(state.completionRunId===runId)hideLayers(); return; }
...
async function finishWorkout(event){
  ...
  }catch(error){state.completionRunId+=1;state.dashboard=dashboardBefore;state.workout=workout;state.steps=stepsBefore;state.gpsDistanceMeters=gpsDistanceBefore;rebuildProvisionalMuscleProgress(workout);cacheDashboard();renderExerciseGrid();renderWorkoutHistory();renderWorkout();if(workout.trackingType==="gps")void startGpsTracking(false);if(workout.trackingType==="steps")void startStepCounter(false);showLayer(elements.finishLayer);elements.finishQuestion.textContent=error instanceof Error?error.message:"Nao foi possivel encerrar.";}
