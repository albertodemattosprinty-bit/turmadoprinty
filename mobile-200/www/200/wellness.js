...
const elements = {
  ...
  activeWorkout:byId("wellnessActiveWorkout"), activeWorkoutImage:byId("wellnessActiveWorkoutImage"), workoutName:byId("wellnessWorkoutName"), workoutCounter:byId("wellnessWorkoutCounter"), workoutDetail:byId("wellnessWorkoutDetail"),
  detail:byId("wellnessExerciseDetail"), detailCategory:byId("wellnessExerciseCategory"), detailName:byId("wellnessExerciseName"), detailFrame:byId("wellnessExerciseDetailFrame"), detailImage:byId("wellnessExerciseDetailImage"), detailMuscleVisual:byId("wellnessExerciseMuscleVisual"), detailMuscleImage:byId("wellnessExerciseMuscleImage"), detailEquipment:byId("wellnessExerciseEquipment"),
  ...
  goalStart:byId("wellnessGoalStart"), workoutLayer:byId("wellnessWorkoutLayer"), phaseLabel:byId("wellnessPhaseLabel"), phaseName:byId("wellnessPhaseExerciseName"),
  phaseNumber:byId("wellnessPhaseNumber"), phaseNumberStepper:byId("wellnessWorkoutNumberStepper"), phaseRepsDecrease:byId("wellnessWorkoutRepsDecrease"), phaseRepsIncrease:byId("wellnessWorkoutRepsIncrease"), phaseUnit:byId("wellnessPhaseUnit"), phaseProgress:byId("wellnessWorkoutProgressRing"), workoutImage:byId("wellnessWorkoutExerciseImage"), workoutSeriesStatus:byId("wellnessWorkoutSeriesStatus"), workoutSeriesCount:byId("wellnessWorkoutSeriesCount"), workoutPrimary:byId("wellnessWorkoutPrimary"), workoutPrimaryLabel:byId("wellnessWorkoutPrimaryLabel"),
  ...
  ,aiPlanLayer:byId("wellnessAiPlanLayer"), aiPlanInput:byId("wellnessAiPlanInput"), aiPlanMic:byId("wellnessAiPlanMic"), aiPlanCreate:byId("wellnessAiPlanCreate"), aiPlanReview:byId("wellnessAiPlanReview"), aiApprovalWrap:byId("wellnessAiApprovalWrap"), aiApprovalInput:byId("wellnessAiApprovalInput"), aiPlanApprove:byId("wellnessAiPlanApprove"), aiPlanStatus:byId("wellnessAiPlanStatus")
};
const phaseLayers = [elements.catalogLayer,elements.detail,elements.goalLayer,elements.workoutLayer,elements.repsLayer,elements.finishLayer,elements.weightLayer,elements.mealConfigLayer,elements.mealAddLayer,elements.aiPlanLayer].filter(Boolean);
const state = { tab:"nutrition", filter:"strength", dashboard:null, isAdmin:false, selectedExercise:null, detailMode:"selected", workout:null, steps:0, lastStepAt:0, motionListening:false, saveTimer:null, ticker:null, exerciseImageTimer:null, exerciseImagePhase:"start", pendingMeal:"", selectedMealSlot:"", gpsWatchId:null, gpsProvider:"", gpsLastPoint:null, gpsDistanceMeters:0, gpsAccuracy:null, gpsStatus:"GPS aguardando localização", gpsPlugin:null, seriesRepsDraft:null, seriesSaveChain:Promise.resolve(), progressAnimationTimer:null, adminImageHoldTimer:null, adminImageHoldTriggered:false, adminImageHoldX:0, adminImageHoldY:0, suppressExerciseImageClick:false, generatingExerciseId:"", aiPlan:null, voiceRecorder:null, voiceStream:null, voiceTarget:null };
...
function exerciseFromLibrary(item){ const source=EXERCISES.find((exercise)=>exercise.id===item?.exerciseId); return source?{...source,library:item}:{id:item?.exerciseId,name:item?.exerciseName||"Exercício",tracking:item?.trackingType||"minutes",equipment:item?.equipment||"Sem equipamento",cue:"Siga o plano aprovado com movimentos controlados e respeite seus limites.",category:item?.category||"strength",library:item}; }
function exerciseImageSources(exercise){ const asset=exerciseAsset(exercise?.id); const fallback="/200/apps/exercicios.png"; return {start:String(asset?.startImageUrl||fallback),finish:String(asset?.finishImageUrl||asset?.startImageUrl||fallback),muscle:String(asset?.muscleImageUrl||"")}; }
function exerciseImageMarkup(exercise,className="wellness-exercise-image"){ const images=exerciseImageSources(exercise); return `<img class="${className}" data-exercise-image data-exercise-image-id="${escapeHtml(exercise?.id||"")}" data-start-src="${escapeHtml(images.start)}" data-finish-src="${escapeHtml(images.finish)}" src="${escapeHtml(state.exerciseImagePhase==="finish"?images.finish:images.start)}" alt="Movimento de ${escapeHtml(exercise?.name||"exercício")}" />`; }
function refreshExerciseImages(){ document.querySelectorAll("[data-exercise-image]").forEach((image)=>{ const source=state.exerciseImagePhase==="finish"?image.dataset.finishSrc:image.dataset.startSrc; if(source&&image.getAttribute("src")!==source)image.setAttribute("src",source); }); }
function startExerciseImageTicker(){ if(state.exerciseImageTimer)return; state.exerciseImageTimer=window.setInterval(()=>{ if(document.hidden||!modal?.classList.contains("active"))return; state.exerciseImagePhase=state.exerciseImagePhase==="start"?"finish":"start"; refreshExerciseImages(); },1500); }
function exerciseTodayValue(item){
  if(!item)return 0;
  const active=state.workout?.exerciseId===item.exerciseId?state.workout:null;
  const snapshot=state.dashboard?.activeWorkout?.exerciseId===item.exerciseId?state.dashboard.activeWorkout:null;
  if(item.trackingType==="series")return Math.max(0,Number(item.todayTotalReps||0));
  if(item.trackingType==="gps"){ const base=Math.max(0,Number(item.todayDistanceMeters||0)); if(!active)return base; return Math.max(0,base-Number(snapshot?.distanceMeters||0)+currentGpsDistance()); }
  const base=Math.max(0,Number(item.todayDurationMinutes||0)); if(!active)return base; const live=Math.max(Number(active.durationMinutes||0),elapsedSeconds(active)/60); return Math.max(0,base-Number(snapshot?.durationMinutes||0)+live);
}
...
function renderExerciseGrid(){
  ...
  const visible=exerciseLibrary().filter((item)=>item.category===category.id).map(exerciseFromLibrary).filter(Boolean);
  ...
  elements.exerciseGrid.innerHTML=visible.map((exercise)=>`<article class="wellness-exercise-item"><button class="wellness-exercise-open" type="button" data-exercise-id="${escapeHtml(exercise.id)}">${exerciseImageMarkup(exercise)}<span class="wellness-exercise-copy"><strong title="${escapeHtml(exercise.name)}">${escapeHtml(exercise.name)}</strong><small>${exerciseMuscles(exercise).map(escapeHtml).join(" · ")}</small></span></button><button class="wellness-exercise-play" type="button" data-exercise-play-id="${escapeHtml(exercise.id)}" aria-label="Iniciar ${escapeHtml(exercise.name)}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m9 7 8 5-8 5V7Z"/></svg></button></article>`).join("");
...
function openExerciseDetail(exercise,mode="selected"){
  ...
  const images=exerciseImageSources(exercise); if(elements.detailImage){ elements.detailImage.dataset.exerciseImage=""; elements.detailImage.dataset.exerciseImageId=exercise.id; elements.detailImage.dataset.startSrc=images.start; elements.detailImage.dataset.finishSrc=images.finish; elements.detailImage.src=state.exerciseImagePhase==="finish"?images.finish:images.start; elements.detailImage.alt=`Movimento de ${exercise.name}`; }
  if(elements.detailMuscleVisual&&elements.detailMuscleImage){ elements.detailMuscleVisual.hidden=!images.muscle; if(images.muscle){ elements.detailMuscleImage.src=images.muscle; elements.detailMuscleImage.alt=`Mapa dos músculos trabalhados em ${exercise.name}`; } }
  const isGenerating=state.generatingExerciseId===exercise.id; elements.detailFrame?.classList.toggle("is-generating",isGenerating);
  if(elements.detailGenerate){ elements.detailGenerate.hidden=!state.isAdmin; elements.detailGenerate.disabled=isGenerating; }
  if(elements.detailGenerateStatus)elements.detailGenerateStatus.textContent=isGenerating?"Gerando as três imagens...":"";
...
async function generateSelectedExerciseImages({skipConfirmation=false}={}){
  ...
  state.generatingExerciseId=exercise.id; elements.detailGenerate.disabled=true; elements.detailFrame?.classList.add("is-generating"); elements.detailGenerateStatus.textContent="Gerando movimento A, movimento B e mapa muscular...";
  try{
...
function beginAdminExerciseImageHold(event){
  ...
  const exerciseId=String(image.dataset.exerciseImageId||state.selectedExercise?.id||""); const exercise=EXERCISES.find((item)=>item.id===exerciseId); if(!exercise)return;
...
function consumeAdminExerciseImageClick(event){ if(!state.suppressExerciseImageClick||!event.target.closest?.("[data-exercise-image]"))return false; state.suppressExerciseImageClick=false; event.preventDefault(); event.stopPropagation(); return true; }
...
async function addSelectedExercise(){
  ...
  try{ const payload=await apiRequest("/api/200/exercises/library",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({profile:profileName(),exerciseId:exercise.id,exerciseName:exercise.name,category:exercise.category,trackingType:exercise.tracking,equipment:exercise.equipment})}); state.dashboard=payload.dashboard||state.dashboard; state.detailMode="selected"; renderExerciseGrid(); renderCatalog(); openExerciseDetail(exercise,"selected"); }
  catch(error){ elements.detailEquipment.textContent=error instanceof Error?error.message:"Não foi possível adicionar."; } finally{ elements.detailStart.disabled=false; }
}
...
function openGoal(){
  ...
  elements.goalStart.textContent=isSeries?"Iniciar série":isGps?"Iniciar atividade":"Iniciar exercício";
  ...
  fillExerciseGoalFields(exercise,item);
  showLayer(elements.goalLayer);
}
function startSelectedExercise(){ const exercise=state.selectedExercise,item=libraryItem(exercise?.id); if(!exercise)return; if(!hasExerciseGoal(item)){ openGoal(); return; } fillExerciseGoalFields(exercise,item); void startExercise({preventDefault(){},currentTarget:elements.detailStart}); }
...
function setWorkoutProgress(value){
  ...
  elements.phaseProgress.classList.remove("is-progressing"); void elements.phaseProgress.offsetWidth; elements.phaseProgress.classList.add("is-progressing");
  if(state.progressAnimationTimer)window.clearTimeout(state.progressAnimationTimer); state.progressAnimationTimer=window.setTimeout(()=>elements.phaseProgress?.classList.remove("is-progressing"),720);
}
function renderWorkout(){
  const workout=state.workout; elements.activeWorkout.hidden=!workout; if(!workout)return;
  const isSeries=workout.trackingType==="series", isGps=workout.trackingType==="gps",exercise=EXERCISES.find((item)=>item.id===workout.exerciseId)||exerciseFromLibrary(libraryItem(workout.exerciseId)||{}),images=exerciseImageSources(exercise),visibleImage=state.exerciseImagePhase==="finish"?images.finish:images.start;
  ...
  if(elements.activeWorkoutImage){ elements.activeWorkoutImage.dataset.exerciseImage=""; elements.activeWorkoutImage.dataset.exerciseImageId=exercise.id; elements.activeWorkoutImage.dataset.startSrc=images.start; elements.activeWorkoutImage.dataset.finishSrc=images.finish; elements.activeWorkoutImage.src=visibleImage; elements.activeWorkoutImage.alt=`Movimento de ${workout.exerciseName||"exercício"}`; }
  if(elements.workoutImage){ elements.workoutImage.dataset.exerciseImage=""; elements.workoutImage.dataset.exerciseImageId=exercise.id; elements.workoutImage.dataset.startSrc=images.start; elements.workoutImage.dataset.finishSrc=images.finish; elements.workoutImage.src=visibleImage; elements.workoutImage.alt=`Movimento de ${workout.exerciseName||"exercício"}`; }
...
async function startExercise(event){
  ...
  catch(error){ const message=error instanceof Error?error.message:"Nao foi possivel iniciar."; if(directStart){ openExerciseDetail(exercise,"selected"); elements.detailEquipment.textContent=message; }else elements.goalForm.querySelector("p").textContent=message; } finally{ submit.disabled=false; }
}
...
async function finishWorkout(event){ event.preventDefault(); if(!state.workout)return; const submit=elements.finishForm.querySelector("button[type=submit]"); submit.disabled=true; try{ await saveWorkoutProgress(); const distanceMeters=state.workout.trackingType==="gps"?currentGpsDistance():state.workout.trackingType==="series"?0:Math.max(0,Math.trunc(Number(elements.distanceInput.value||0)||0)); if(isOfflineWorkout()){ const completed={...state.workout,steps:state.steps,distanceMeters,completedAt:new Date().toISOString(),status:"completed"}; persistOfflineWorkout(completed,{finished:true,finishPayload:{steps:state.steps,distanceMeters}}); state.dashboard={...(state.dashboard||{}),activeWorkout:null,recentWorkouts:[completed,...(state.dashboard?.recentWorkouts||[])]}; cacheDashboard(); }else{ await apiRequest(`/api/200/exercises/${encodeURIComponent(state.workout.id)}/finish`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({steps:state.steps,distanceMeters})}); } await stopGpsTracking(); stopStepCounter(); state.workout=null; state.steps=0; state.gpsDistanceMeters=0; hideLayers(); if(navigator.onLine!==false)await loadDashboard(); else{renderExerciseGrid();renderWorkoutHistory();renderWorkout();} }catch(error){ elements.finishQuestion.textContent=error instanceof Error?error.message:"Nao foi possivel encerrar."; }finally{ submit.disabled=false; } }
...
function voiceStatus(){ return state.voiceTarget==="plan"?elements.aiPlanStatus:elements.mealAddStatus; }
...
function openAiPlan(){ state.aiPlan=null; elements.aiPlanReview.hidden=true; elements.aiApprovalWrap.hidden=true; elements.aiPlanApprove.hidden=true; elements.aiApprovalInput.value=""; elements.aiPlanStatus.textContent=""; showLayer(elements.aiPlanLayer); }
function renderAiPlan(){ const plan=state.aiPlan; if(!plan)return; elements.aiPlanReview.hidden=false; elements.aiPlanReview.innerHTML=`<header><strong>${escapeHtml(plan.name||"Plano de exercícios")}</strong><p>${escapeHtml(plan.summary||"")}</p></header>${(plan.exercises||[]).map((item,index)=>`<article><b>${index+1}</b><div><strong>${escapeHtml(item.exerciseName)}</strong><small>${item.trackingType==="series"?`${item.targetSeries} séries de ${item.targetReps}`:item.trackingType==="gps"?`${Number(item.targetDistanceMeters||0)/1000} km`:`${item.targetMinutes} minutos`} · ${escapeHtml(item.equipment||"Sem equipamento")}</small></div></article>`).join("")}`; elements.aiApprovalWrap.hidden=false; elements.aiPlanApprove.hidden=false; }
async function createAiPlan(){ const description=String(elements.aiPlanInput.value||"").trim(); if(description.length<8){ elements.aiPlanStatus.textContent="Conte um pouco mais sobre o plano que deseja."; return; } elements.aiPlanCreate.disabled=true; elements.aiPlanStatus.textContent="A IA está montando seu plano..."; try{ const payload=await apiRequest("/api/200/exercises/plan",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({description})}); state.aiPlan=payload.plan; renderAiPlan(); elements.aiPlanStatus.textContent="Revise a lista. Nada foi adicionado ainda."; }catch(error){ elements.aiPlanStatus.textContent=error instanceof Error?error.message:"Não foi possível montar."; }finally{ elements.aiPlanCreate.disabled=false; } }
async function approveAiPlan(){ const approval=String(elements.aiApprovalInput.value||"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").trim().toLowerCase(); if(approval!=="aprovar"){ elements.aiPlanStatus.textContent='Digite "Aprovar" para confirmar o pacote.'; return; } elements.aiPlanApprove.disabled=true; elements.aiPlanStatus.textContent="Adicionando ao seu pacote..."; try{ const payload=await apiRequest("/api/200/exercises/plan/approve",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({profile:profileName(),approval:"Aprovar",exercises:state.aiPlan?.exercises||[]})}); state.dashboard=payload.dashboard||state.dashboard; hideLayers(); renderExerciseGrid(); }catch(error){ elements.aiPlanStatus.textContent=error instanceof Error?error.message:"Não foi possível aprovar."; }finally{ elements.aiPlanApprove.disabled=false; } }
...
elements.exerciseGrid?.addEventListener("click",(event)=>{ if(consumeAdminExerciseImageClick(event))return; const play=event.target.closest("[data-exercise-play-id]"); if(play){ const exercise=EXERCISES.find((item)=>item.id===play.dataset.exercisePlayId); if(!exercise)return; if(state.workout){ renderWorkout(); showLayer(elements.workoutLayer); return; } state.selectedExercise=exercise; state.detailMode="selected"; startSelectedExercise(); return; } const target=event.target.closest("[data-exercise-id],[data-open-catalog]"); if(!target)return; if(target.hasAttribute("data-open-catalog")){ openCatalog(); return; } const exercise=EXERCISES.find((item)=>item.id===target.dataset.exerciseId); if(exercise)openExerciseDetail(exercise,"selected"); });
elements.catalogGrid?.addEventListener("click",(event)=>{ if(consumeAdminExerciseImageClick(event))return; const exercise=EXERCISES.find((item)=>item.id===event.target.closest("[data-catalog-exercise-id]")?.dataset.catalogExerciseId); if(exercise)openExerciseDetail(exercise,"catalog"); });
...
[elements.exerciseGrid,elements.catalogGrid,elements.detailFrame].filter(Boolean).forEach((surface)=>{
...
elements.detailFrame?.addEventListener("click",(event)=>{ consumeAdminExerciseImageClick(event); });
...
byId("wellnessAiPlanOpen")?.addEventListener("click",openAiPlan);
byId("wellnessAiPlanClose")?.addEventListener("click",hideLayers);
elements.aiPlanMic?.addEventListener("click",()=>void toggleWellnessVoice("plan"));
elements.aiPlanCreate?.addEventListener("click",()=>void createAiPlan());
elements.aiPlanApprove?.addEventListener("click",()=>void approveAiPlan());
