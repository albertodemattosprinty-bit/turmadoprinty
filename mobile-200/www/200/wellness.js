...
const elements = {
  ...
  workoutFinish:byId("wellnessWorkoutFinish"), workoutCancel:byId("wellnessWorkoutCancel"), repsLayer:byId("wellnessRepsLayer"), repsForm:byId("wellnessRepsForm"), repsInput:byId("wellnessRepsInput"),
...
function configureFixedExerciseMedia(shell,image,video,exercise,{movementOnly=false}={}){
  ...
  shell.dataset.mediaSignature=signature; shell.classList.toggle("has-video",hasVideo); shell.dataset.exerciseId=String(exercise?.id||"");
...
function openExerciseDetail(exercise,mode="selected"){
  ...
  updateExerciseProgressDetail(); renderExerciseMuscleMaps(exercise); showLayer(elements.detail); elements.detail?.querySelector(".wellness-phase-screen")?.scrollTo({top:0});
}
...
function validExerciseDefinition(exercise){ return exerciseMuscleLoads(exercise).some((item)=>isMuscleLoadDefined(item.load)); }
...
async function deleteWorkoutHistoryEntry(workout){
  ...
  try{const payload=await apiRequest(`/api/200/exercises/${encodeURIComponent(workout.id)}/history`,{method:"DELETE",forceNetwork:true,skipGlobalLoading:true,offlineInvalidates:["/api/200/wellness","/api/200/extra-goals"]});state.dashboard=withoutDeletedWorkoutHistory(payload.dashboard||state.dashboard);cacheDashboard();renderExerciseGrid();renderWorkoutHistory();renderMuscleProgressMap();window.dispatchEvent(new CustomEvent("project200:exercise-mission-updated"));}
...
function renderWorkout(){
  ...
  elements.phaseNumber.classList.toggle("wellness-phase-distance",isGps); elements.phaseNumberStepper.classList.toggle("is-series",isSeries); elements.phaseNumberStepper.hidden=!isSeries; elements.phaseNumber.hidden=!isSeries; elements.phaseRepsDecrease.hidden=!isSeries; elements.phaseRepsIncrease.hidden=!isSeries; elements.workoutSeriesStatus.hidden=!isSeries;
  ...
  if(isSeries){ const done=Number(workout.seriesCount||0), target=Math.max(1,Number(workout.targetSeries||1)), reps=currentSeriesReps(); elements.phaseLabel.textContent=done>=target?"Meta concluída · série extra":`Série ${done+1} de ${target}`; renderWorkoutSeriesMarkers(done,target); elements.phaseNumber.textContent=String(reps); elements.phaseUnit.textContent=""; elements.workoutPrimaryLabel.textContent=`Adicionar série de ${reps} movimentos`; elements.workoutPrimary.setAttribute("aria-label",elements.workoutPrimaryLabel.textContent); setWorkoutProgress((done/target)*100); return; }
...
async function finishWorkout(){
  ...
  try{
    ...
    else serverFinish=apiRequest(`/api/200/exercises/${encodeURIComponent(workout.id)}/finish`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({steps:state.steps,distanceMeters,series}),offlineInvalidates:["/api/200/wellness"],skipGlobalLoading:true});
...
elements.phaseProgress?.addEventListener("click",(event)=>{if(event.target.closest("button"))return;toggleExerciseVideo(elements.phaseProgress);});
...
elements.workoutCancel?.addEventListener("click",openCancelWorkout);
