...
async function startExercise(event){
  ...
  try{ const payload=await apiRequest("/api/200/exercises/start",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({profile:profileName(),exerciseId:exercise.id,exerciseName:exercise.name,category:exercise.category,trackingType:exercise.tracking,equipment:exercise.equipment,targetSeries:isSeries?targetSeries:0,targetReps:isSeries?targetReps:0,targetMinutes:!isSeries&&!isGps?targetMinutes:0,targetDistanceMeters:isGps?targetDistanceMeters:0})}); state.dashboard=payload.dashboard||state.dashboard; state.workout=payload.workout; state.seriesRepsDraft=null; state.steps=Number(state.workout?.steps||0); state.gpsDistanceMeters=Number(state.workout?.distanceMeters||0); if(exercise.tracking==="steps")await startStepCounter(true); if(exercise.tracking==="gps")await startGpsTracking(true); renderExerciseGrid(); renderWorkout(); showLayer(elements.workoutLayer); }
...
async function finishWorkout(event){ event.preventDefault(); if(!state.workout)return; const submit=elements.finishForm.querySelector("button[type=submit]"); submit.disabled=true; try{ await saveWorkoutProgress(); const distanceMeters=state.workout.trackingType==="gps"?currentGpsDistance():state.workout.trackingType==="series"?0:Math.max(0,Math.trunc(Number(elements.distanceInput.value||0)||0)); await apiRequest(`/api/200/exercises/${encodeURIComponent(state.workout.id)}/finish`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({steps:state.steps,distanceMeters})}); await stopGpsTracking(); stopStepCounter(); state.workout=null; state.steps=0; state.gpsDistanceMeters=0; hideLayers(); await loadDashboard(); }catch(error){ elements.finishQuestion.textContent=error instanceof Error?error.message:"Nao foi possivel encerrar."; }finally{ submit.disabled=false; } }
async function discardWorkout(){ if(!state.workout)return; elements.discardWorkout.disabled=true; try{ await apiRequest(`/api/200/exercises/${encodeURIComponent(state.workout.id)}`,{method:"DELETE"}); await stopGpsTracking(); stopStepCounter(); state.workout=null; state.steps=0; state.gpsDistanceMeters=0; state.gpsLastPoint=null; hideLayers(); await loadDashboard(); }catch(error){ elements.finishQuestion.textContent=error instanceof Error?error.message:"Nao foi possivel excluir."; }finally{ elements.discardWorkout.disabled=false; } }
