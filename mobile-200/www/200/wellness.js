...
const elements = {
  ...
  detail:byId("wellnessExerciseDetail"), detailCategory:byId("wellnessExerciseCategory"), detailName:byId("wellnessExerciseName"), detailFrame:byId("wellnessExerciseDetailFrame"), detailImage:byId("wellnessExerciseDetailImage"), detailEquipment:byId("wellnessExerciseEquipment"),
  detailInstructions:byId("wellnessExerciseInstructions"), detailStart:byId("wellnessExerciseStart"), detailGoal:byId("wellnessExerciseGoal"), detailProgress:byId("wellnessExerciseProgress"), detailProgressText:byId("wellnessExerciseProgressText"),
...
const phaseLayers = [elements.catalogLayer,elements.detail,elements.goalLayer,elements.workoutLayer,elements.repsLayer,elements.finishLayer,elements.weightLayer,elements.mealConfigLayer,elements.mealAddLayer].filter(Boolean);
const EXERCISE_IMAGE_PHASES = ["start","finish","muscle"];
const state = { tab:"nutrition", filter:"strength", dashboard:null, isAdmin:false, selectedExercise:null, detailMode:"selected", goalEditMode:false, workout:null, steps:0, lastStepAt:0, motionListening:false, saveTimer:null, ticker:null, exerciseImageTimer:null, exerciseMuscleTimer:null, exerciseImagePhase:"start", muscleNameIndex:0, pendingMeal:"", selectedMealSlot:"", gpsWatchId:null, gpsProvider:"", gpsLastPoint:null, gpsDistanceMeters:0, gpsAccuracy:null, gpsStatus:"GPS aguardando localização", gpsPlugin:null, seriesRepsDraft:null, seriesSaveChain:Promise.resolve(), progressAnimationTimer:null, adminImageHoldTimer:null, adminImageHoldTriggered:false, adminImageHoldX:0, adminImageHoldY:0, suppressExerciseImageClick:false, generatingExerciseId:"", voiceRecorder:null, voiceStream:null, voiceTarget:null };
...
function exerciseImageSources(exercise){ const asset=exerciseAsset(exercise?.id); const fallback="/200/apps/exercicios.png",start=String(asset?.startImageUrl||fallback),finish=String(asset?.finishImageUrl||asset?.startImageUrl||fallback); return {start,finish,muscle:String(asset?.muscleImageUrl||finish)}; }
...
function exerciseMuscleMarkup(exercise){ const muscles=exerciseMuscles(exercise); return `<small data-exercise-muscles="${escapeHtml(JSON.stringify(muscles))}">${escapeHtml(muscles[state.muscleNameIndex%muscles.length]||"Corpo inteiro")}</small>`; }
function refreshExerciseMuscleLabels(){ document.querySelectorAll("[data-exercise-muscles]").forEach((label)=>{ try{ const muscles=JSON.parse(label.dataset.exerciseMuscles||"[]"); if(Array.isArray(muscles)&&muscles.length)label.textContent=String(muscles[state.muscleNameIndex%muscles.length]||""); }catch{} }); }
...
function dailyExerciseProgress(){ const items=exerciseLibrary(); if(!items.length)return 0; return Math.round(items.reduce((sum,item)=>sum+Math.min(100,exerciseProgress(item).percent),0)/items.length); }
...
function openGoal({editing=false}={}){
  ...
  elements.goalStart.textContent=editing?"Continuar para periodicidade":isSeries?"Iniciar série":isGps?"Iniciar atividade":"Iniciar exercício";
  elements.goalHelp.textContent=editing?"Ajuste a meta e, no próximo passo, escolha quando deseja repetir este exercício.":"Você pode fazer em várias etapas ao longo do dia ou encerrar antes. Tudo realizado será somado na sua tabela.";
...
function renderWorkout(){
  ...
  const isSeries=workout.trackingType==="series", isGps=workout.trackingType==="gps",exercise=EXERCISES.find((item)=>item.id===workout.exerciseId)||exerciseFromLibrary(libraryItem(workout.exerciseId)||{}),images=exerciseImageSources(exercise),visibleImage=exerciseImageForPhase(images);
...
async function loadDashboard(){
  try{ const payload=await apiRequest(dashboardPath(),{cache:"no-store"}); state.dashboard=payload?.dashboard||{}; applyAdminAccess(payload?.isAdmin); const local=readOfflineWorkouts().find((item)=>!item.finished)?.workout||null; state.workout=local||state.dashboard.activeWorkout||null; if(local)state.dashboard.activeWorkout=local; state.seriesRepsDraft=null; state.steps=Number(state.workout?.steps||0); state.gpsDistanceMeters=Number(state.workout?.distanceMeters||0); renderMeals(); renderWeight(); renderExerciseGrid(); renderWorkoutHistory(); renderWorkout(); if(!elements.catalogLayer?.hidden)renderCatalog(); if(!elements.detail?.hidden&&state.selectedExercise)openExerciseDetail(state.selectedExercise,state.detailMode); if(state.workout?.trackingType==="steps")void startStepCounter(false); if(state.workout?.trackingType==="gps")void startGpsTracking(false); window.dispatchEvent(new CustomEvent("project200:exercise-mission-updated")); }
...
async function saveExerciseGoal(scheduleConfig){
  const exercise=state.selectedExercise;if(!exercise)return;
  const help=elements.goalForm?.querySelector("p"); elements.goalStart.disabled=true; if(help)help.textContent="Salvando meta e periodicidade...";
  try{ const payload=await apiRequest("/api/200/exercises/library",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({profile:profileName(),exerciseId:exercise.id,exerciseName:exercise.name,category:exercise.category,trackingType:exercise.tracking,equipment:exercise.equipment,...exerciseGoalValues(exercise),scheduleConfig})}); state.dashboard=payload.dashboard||state.dashboard; state.goalEditMode=false; renderExerciseGrid(); openExerciseDetail(exercise,"selected"); window.dispatchEvent(new CustomEvent("project200:exercise-mission-updated")); }
...
function submitExerciseGoal(event){
  if(!state.goalEditMode){ void startExercise(event); return; }
  event.preventDefault(); const exercise=state.selectedExercise,item=libraryItem(exercise?.id); if(!exercise)return;
  const scheduleModal=window.project200DailyRepetitionModal;
  if(!scheduleModal?.open){ void saveExerciseGoal(item?.scheduleConfig||null); return; }
  scheduleModal.open("exercise-goal",(schedule)=>void saveExerciseGoal(schedule),item?.scheduleConfig||null);
}
